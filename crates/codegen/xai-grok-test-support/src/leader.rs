...
impl LeaderFixture {
    ...
    async fn start_with_binary_base_url_timeout(
        ...
        cmd.envs(xai_tty_utils::pager_env())
            .env("CODING_AGENT_API_BASE_URL", base_url)
            .env("CODING_AGENT_MODELS_BASE_URL", base_url)
            .env("GROK_FEEDBACK_BASE_URL", base_url)
            .env("GROK_TRACE_UPLOAD_URL", base_url)
            .env("CODING_AGENT_API_KEY", "test-key-for-ci")
...
impl LeaderStdioClient {
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init
            .auth_methods
            .iter()
            .find(|method| &*method.id().0 == "api_key")
