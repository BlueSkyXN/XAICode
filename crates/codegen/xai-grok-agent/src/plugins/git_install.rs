...
pub fn validate_git_url(url: &str) -> Result<&str, String> {
    ...
    if is_removed_vendor_git_url(url) {
        return Err("vendor-owned xAI/Grok plugin sources are disabled".to_owned());
    }
    Ok(url)
}

/// Reject the former vendor's repositories and service hosts at the lowest
/// common clone boundary. Marketplace callers have their own deny-list, but
/// direct `plugin install <url>` and refresh paths must be fail-closed too.
fn is_removed_vendor_git_url(value: &str) -> bool {
    let lower = value.trim().to_ascii_lowercase();
    if lower.contains("github.com/xai-org/") || lower.contains("github.com:xai-org/") {
        return true;
    }
    let rest = lower
        .strip_prefix("https://")
...
pub fn update_repo(
    ...
    match &repo.kind {
        ...
        InstallKind::Git {
            ...
            // A registry created by an older build may still contain a
            // vendor-owned remote. Re-validate before any fetch/pull so an
            // update cannot bypass the clean-build deny-list.
            validate_git_url(url).map_err(|detail| InstallError::InstallFailed { detail })?;
            // Check if pinned
            if let Some(r) = git_ref {
...
mod tests {
    ...
    #[test]
    fn git_operand_validator_rejects_removed_vendor_sources() {
        for url in [
            "https://api.x.ai/plugins.git",
            "https://plugins.grok.com/catalog.git",
            "https://github.com/xai-org/first-party-plugin.git",
            "git@github.com:xai-org/first-party-plugin.git",
        ] {
            assert!(
                validate_git_url(url).is_err(),
                "vendor URL {url:?} must fail"
            );
        }
    }
