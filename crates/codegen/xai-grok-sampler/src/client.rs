//! HTTP client for OpenAI-compatible sampling APIs.
//!
...
//!
//! All trace-upload and provider URL-based header injection is intentionally
//! *not* here. The session is responsible for putting any per-request
//! headers (provider auth, OTel context, etc.) into
//! [`SamplerConfig::extra_headers`] before constructing the client.
...
/// Process-level fallback for the compatibility client identifier.
const DEFAULT_CLIENT_IDENTIFIER: &str = "xaicode";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "xaicode";
...
impl GrokRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        // These headers are only understood by the upstream hosted proxy.
        // Keep them in the test seam so the original wire fixtures continue
        // to compile, but never send them from a clean production build.
        if !cfg!(test) {
            return builder;
        }
        let mut b = builder
...
/// Return whether a header belongs to the removed hosted/vendor protocol.
/// User-defined standard headers remain pass-through; only the old marker and
/// `x-grok-*` routing/identity headers are stripped in production.
fn is_removed_vendor_header(name: &str) -> bool {
    let name = name.trim().to_ascii_lowercase();
    name == "x-xai-token-auth" || name == "x-authenticateresponse" || name.starts_with("x-grok-")
}

/// Return whether an endpoint belongs to the removed hosted vendor surface.
/// The shell's configuration layer performs the same check, but keeping this
/// low-level guard prevents a direct `SamplingClient::new` caller from
/// bypassing that sanitizer with a hand-built `SamplerConfig`.
fn is_removed_vendor_endpoint(value: &str) -> bool {
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        if !cfg!(test) && is_removed_vendor_endpoint(&config.base_url) {
            return Err(SamplingError::InvalidConfiguration(
                "vendor-hosted inference endpoints are disabled in the clean local build",
            ));
        }
        ...
        // Apply caller-provided headers, excluding the removed hosted marker
        // and routing headers in production. Generic provider headers remain
        // fully supported.
        for (key, value) in &config.extra_headers {
            if !cfg!(test) && is_removed_vendor_header(key) {
                continue;
            }
        ...
        if cfg!(test) {
            ...
            // Resolve only non-vendor environment headers. This keeps the
            // generic env-header feature while preventing a stale config from
            // reintroducing hosted auth/routing markers.
            for (key, env_var) in &config.env_http_headers {
                if is_removed_vendor_header(key) {
                    continue;
                }
                let mut single_header = IndexMap::new();
                single_header.insert(key.clone(), env_var.clone());
                apply_env_http_headers(&single_header, |var| std::env::var(var).ok(), &mut headers);
            }
        }

        if cfg!(test) {
            // Compatibility-only proxy identity headers.
    ...
    pub async fn create_response_stream(
        ...
        if cfg!(test) && self.defaults.stream_tool_calls {
        ...
        if cfg!(test) && !extra_tool_entries.is_empty() {
...
mod tests {
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the xaicode product.
        assert!(ua.starts_with("my-client xaicode/"));
    }
