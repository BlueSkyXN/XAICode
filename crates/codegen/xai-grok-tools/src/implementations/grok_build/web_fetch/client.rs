...
impl WebFetchClient {
    pub fn new(params: &WebFetchParams) -> Result<Self, WebFetchError> {
        if let Some(proxy) = params.proxy_endpoint.as_deref()
            && let Ok(proxy_url) = Url::parse(proxy)
            && let Some(host) = proxy_url.host_str()
            && is_removed_vendor_host(host)
        {
            return Err(WebFetchError::RemovedVendorDomain {
...
/// The clean build must not use the generic web fetch tool to reopen the
/// removed vendor product surfaces, even when a caller supplies a custom
/// allowlist that contains one of those domains.
...
mod tests {
    ...
    #[test]
    fn validate_url_rejects_removed_vendor_domains() {
        for value in [
            "https://x.ai/docs",
            "https://api.x.ai/v1",
            "https://grok.com",
            "https://console.grok.build/billing",
            "https://grok.ai",
        ] {
            assert!(matches!(
                validate_url(value),
                Err(WebFetchError::RemovedVendorDomain { .. })
            ));
        }
        assert!(validate_url("https://x.ai.evil.example/docs").is_ok());
    }
