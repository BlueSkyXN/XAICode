...
pub(crate) const TIER_RESTRICTED_UPSELL: &str =
    "Image generation is disabled in the clean local build. Do not retry this tool.";
...
impl ImageGenClient {
    ...
    pub fn new(
        ...
        if !cfg!(test) {
            let _ = (config, &api_key_provider);
            return Err(xai_tool_runtime::ToolError::invalid_arguments(
                "Image generation is disabled in the clean local build",
            ));
        }
    ...
    pub async fn generate(
        ...
        if !cfg!(test) {
            let _ = (self, prompt, aspect_ratio);
            return Err(xai_tool_runtime::ToolError::invalid_arguments(
                "Image generation is disabled in the clean local build",
            ));
        }
...
// The upstream tests construct a live vendor image client. Image generation
// is deliberately removed from the clean build, so keep those tests as source
// reference without compiling them against the disabled constructor.
#[cfg(any())]
mod tests {
    use super::*;
