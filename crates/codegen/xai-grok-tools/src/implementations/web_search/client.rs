...
impl WebSearchClient {
    ...
    pub fn new(
        ...
        // Web search is an upstream hosted capability, not part of the local
        // coding-agent core. Keep the constructor/test seam so downstream
        // crates and fixtures retain their original shape, while production
        // cannot construct a client that could issue a provider request.
        if !cfg!(test) {
            return Err(xai_tool_runtime::ToolError::execution(
                xai_tool_protocol::ToolId::new("web_search").expect("valid"),
                "Web search is disabled in the clean local build".to_string(),
            ));
        }
            ...
            .any(|domain| endpoint.contains(domain))
        {
            return Err(xai_tool_runtime::ToolError::execution(
                xai_tool_protocol::ToolId::new("web_search").expect("valid"),
                "Hosted vendor web search is disabled in the clean local build".to_string(),
            ));
        }
    ...
    pub async fn search(
        ...
        if !cfg!(test) {
            let _ = (self, query, &allowed_domains);
            return Err(xai_tool_runtime::ToolError::execution(
                xai_tool_protocol::ToolId::new("web_search").expect("valid"),
                "Web search is disabled in the clean local build".to_string(),
            ));
...
mod tests {
    ...
    fn test_new_client_uses_configured_model() {
        let config = WebSearchConfig::Enabled {
            ...
            base_url: "https://provider.example.invalid/v1".to_string(),
