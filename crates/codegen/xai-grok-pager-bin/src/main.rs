...
use xai_grok_pager::app::{
    ...
    LeaderTargetArgs, PagerArgs, resolve_leader_mode, warn_leader_disabled_by_sandbox,
};
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    // Hosted relay URL overrides were intentionally removed.  Headless and
    // serve modes now use the local ACP/stdin transport; keep this seam so
    // downstream integrations that construct `HeadlessArgs` remain source
    // compatible while their legacy values are ignored.
    let _ = (args, config);
}
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   Xaicode server starting...");
    eprintln!();
...
/// Managed deployment setup is not part of the local build.
async fn run_setup_command(json: bool) {
    let _ = json;
    eprintln!("Managed configuration and account setup are disabled in the clean build.");
    eprintln!("Configure a local provider in config.toml or set CODING_AGENT_API_KEY.");
}
...
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "xaicode: --plugin-dir is ignored in leader mode; run with --no-leader to \
...
async fn run_agent_command(
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            shutdown_cleanly(code);
        }
        #[cfg(not(unix))]
        {
            if tokio::signal::ctrl_c().await.is_ok() {
                shutdown_cleanly(130);
            }
        }
    });
    if matches!(
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Xaicode - v{}",
            xai_grok_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), "local")
        );
    }
    // Startup never contacts a hosted settings/model-control plane.  Models
    // come exclusively from local config and explicit provider endpoints.
    ...
    // Shared leader/relay mode is a hosted product feature.  The clean build
    // always keeps the agent in its own local process and stdio transport.
    let use_leader = false;
    let policy_disable_reason = Some("disabled in clean build");
    let disabled_by_confinement = None;
    tracing::info!(
        use_leader,
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Leader(a)) => {
            let _ = a;
            anyhow::bail!("leader/relay mode is disabled in the clean local build")
        }
        None => {
            let mut agent_config = agent_config.clone();
...
/// Sets `CODING_AGENT_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `xaicode dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `xaicode dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `CODING_AGENT_DASHBOARD=0`) is a CLI
    ...
    if !xai_grok_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             ...
             unsetting CODING_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("CODING_AGENT_OPEN_DASHBOARD_AT_STARTUP", "1") };
    #[cfg(test)]
    {
        // Keep the historical test seam while production uses the generic
        // Xaicode variable.
    ...
    fn notice(&self) -> Option<String> {
        match self {
            ...
            Self::Clamped {
                ...
                "xaicode: clamped {GROK_WORKER_THREADS_ENV}={requested} to {used} (valid range is 1..={cores})"
            )),
            Self::Ignored { value, .. } => Some(format!(
                "xaicode: ignoring {GROK_WORKER_THREADS_ENV}={value:?} (not a valid integer)"
            )),
        }
    }
}
...
fn dispatch_version_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = write_version(&mut std::io::stdout().lock(), "local") {
...
fn main() {
    ...
    if xai_grok_shell::util::config::load_crash_handler_enabled_sync() {
        ...
        if let Some(report) = xai_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("Xaicode crashed during your last session.");
    ...
    let runtime = tokio::runtime::Builder::new_multi_thread()
        ...
        .build()
        .unwrap_or_else(|e| {
            eprintln!("xaicode: failed to start tokio runtime with {workers} workers: {e}");
            shutdown_cleanly(1);
        });
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    xai_grok_shell::http::set_client_name(xai_grok_workspace::permission::ClientType::Generic);
        ...
        match command {
            Command::Version { json } => {
                if json {
                    ...
                    write_version(&mut std::io::stdout().lock(), "local")?;
                }
                return Ok(());
            }
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `xaicode agent {flag}` instead."
                    );
                }
            ...
            Command::Leader(_leader_args) => {
                return Err(anyhow::anyhow!(
                    "leader/relay management is disabled in the clean local build"
                ));
            }
...
mod tests {
    ...
    fn override_out_of_range_is_clamped_with_a_notice() {
        ...
        assert_eq!(
            ...
            "xaicode: clamped GROK_WORKER_THREADS=100000 to 360 (valid range is 1..=360)"
        );
    }
    #[test]
    fn override_unparseable_is_ignored_with_a_notice() {
        ...
        assert_eq!(
            ...
            "xaicode: ignoring GROK_WORKER_THREADS=\"abc\" (not a valid integer)"
        );
    }
    #[test]
    fn version_output_writer_preserves_channel_aware_contract() {
        for (label, expected_suffix) in [
            ...
            assert!(output.starts_with("xaicode "));
