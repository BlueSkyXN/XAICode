//! Minimal-mode provider setup / folder-trust rendering for the live region.
//!
//! Before any agent session exists (unauthenticated / folder-trust pending) the
//! minimal live region shows provider setup state, a configuration error, the
//! folder-trust question, or a brief "starting"
...
/// in-region provider setup state, a configuration error, the
...
pub(super) enum MinimalAuthHint {
    /// Compatibility state from the upstream auth model. The clean renderer
    /// never opens the URL or browser; it shows local provider setup guidance.
    SigningIn {
        url: Option<String>,
        code: Option<String>,
    },
...
pub(super) fn minimal_auth_hint(
    ...
    match auth {
        AuthState::Authenticating { .. } => MinimalAuthHint::SigningIn {
            // Do not surface or follow stale upstream verification URLs.
            url: None,
            code: None,
        },
        AuthState::Pending { error: Some(err) } => MinimalAuthHint::Failed(err.clone()),
        // A stale upstream login state is rendered as local provider setup;
        // the clean composition root never starts the flow.
...
pub(super) fn auth_hint_rows(hint: &MinimalAuthHint, width: u16) -> u16 {
    match hint {
        ...
        // "Provider setup failed" + blank + error
    ...
    match hint {
        MinimalAuthHint::SigningIn { .. } => {
            y = put_line(
                ...
                y,
                bottom,
                Line::from(Span::styled("Configure a provider API key", bold)),
            );
            ...
            let _ = put_line(
                ...
                Line::from(Span::styled(
                    "Set CODING_AGENT_API_KEY and CODING_AGENT_API_BASE_URL, then restart.",
                    gray,
                )),
            );
        }
        MinimalAuthHint::Failed(err) => {
            ...
            y = put_line(
                ...
                y,
                bottom,
                Line::from(Span::styled("Provider setup failed", warn)),
            );
            y = put_line(buf, area, y, bottom, Line::default());
            let _ = put_line(
                buf,
        ...
        MinimalAuthHint::TrustFolder { workspace } => {
            ...
            y = put_line(
                ...
                bottom,
                Line::from(Span::styled(
                    "Xaicode may run or modify contents in this directory,",
                    gray,
                )),
            );
            y = put_line(
        ...
        MinimalAuthHint::Starting => {
            let _ = put_line(
                ...
                y,
                bottom,
                Line::from(Span::styled("Starting local session\u{2026}", gray)),
            );
        }
    }
}
...
mod tests {
    ...
    fn auth_hint_maps_auth_state() {
        ...
        // Any stale upstream device-flow state is rendered as local setup,
        // without exposing or opening the verification URL.
        ...
        // External command flow also remains URL-free.
        let st = AuthState::Authenticating {
            request_seq: 2,
            handle: None,
    ...
    #[test]
    fn render_auth_shows_provider_setup_hint() {
        let theme = Theme::current();
        let area = Rect::new(0, 0, 80, 12);
        let mut buf = Buffer::empty(area);
        let hint = MinimalAuthHint::SigningIn {
            url: Some("https://provider.example.invalid/device?user_code=ABCD-EFGH".into()),
            code: Some("ABCD-EFGH".into()),
        };
        render_auth(&mut buf, area, &theme, &hint);
        let text = buffer_text(&buf, area);
        assert!(
            text.contains("Configure a provider API key"),
            "header: {text:?}"
        );
        assert!(
            !text.contains("provider.example.invalid"),
            "stale URL: {text:?}"
        );
        assert!(!text.contains("ABCD-EFGH"), "stale device code: {text:?}");
        assert!(
            text.contains("CODING_AGENT_API_KEY"),
            "provider hint: {text:?}"
        );
    }
