...
//! Backend environment presets retained for the upstream crate graph.
//!
//! The clean build has no hosted backend preset. All compiled defaults point
//! at a local sentinel so a legacy relay/leader caller cannot inherit an xAI
//! endpoint merely by selecting the historical `Production` variant.
...
const PRODUCTION_ENDPOINTS: GrokBuildEndpoints = GrokBuildEndpoints {
    cli_chat_proxy_base_url: "http://127.0.0.1:8000/v1",
    asset_server_url: "http://127.0.0.1:8000/assets",
    relay_ws_url: "ws://127.0.0.1:8000/disabled-relay",
    gateway_ws_url: "ws://127.0.0.1:8000/disabled-gateway",
    ws_origin: "http://127.0.0.1:8000",
};
...
impl GrokBuildEnvironment {
    ...
    /// Legacy relay WebSocket URL. It is a local disabled sentinel in this
    /// build and is never opened by the clean composition roots.
    pub fn relay_ws_url(&self) -> String {
        self.resolve("_WS_URL", self.endpoints().relay_ws_url)
    }
    /// Legacy gateway WebSocket URL. It is a local disabled sentinel in this
    /// build and is never opened by the clean composition roots.
...
mod tests {
    ...
    /// loop mistakenly connecting to a hosted relay).
    #[test]
