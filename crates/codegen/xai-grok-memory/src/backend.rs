...
impl EndpointScopedCredentials {
    ...
    pub fn for_endpoint(
        ...
        if !cfg!(test) && is_removed_vendor_endpoint(endpoint) {
            return Self::none();
        }
    ...
    fn approved_for(&self, base_url: &str) -> bool {
        if !cfg!(test) && is_removed_vendor_endpoint(base_url) {
            return false;
        }
        match &self.endpoint {
            None => self.is_empty(),
...
async fn build_embedding_provider(
    ...
    // A stale memory configuration must not turn the embedding client into a
    // backdoor to the removed hosted provider. Generic OpenAI-compatible or
    // local endpoints remain supported; only the historical vendor domains
    // are rejected here.
    if !cfg!(test) && is_removed_vendor_endpoint(base_url) {
        return None;
    }
