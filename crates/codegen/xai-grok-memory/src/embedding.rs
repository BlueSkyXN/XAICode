...
impl ApiEmbeddingProvider {
    ...
    pub fn from_config(
        ...
        if !cfg!(test) && is_removed_vendor_endpoint(&api_base) {
            tracing::warn!(api_base, "memory embeddings: vendor endpoint disabled");
            return None;
        }
...
impl EmbeddingProvider for ApiEmbeddingProvider {
    ...
    async fn embed_batch(
        ...
        if !cfg!(test) && is_removed_vendor_endpoint(&self.api_base) {
            return Err("vendor embedding endpoints are disabled in the clean local build".into());
        }
        if texts.is_empty() {
            return Ok(vec![]);
        }
        ...
        for batch in texts.chunks(self.max_batch_size) {
            ...
            for attempt in 0..MAX_RETRIES {
                ...
                let request = xai_grok_http::shared_client()
                    ...
                    .json(&body_json);
                // The historical mock seam expects the old marker, but a
                // clean production request carries no vendor identity header.
                let request = if cfg!(test) {
                    request.header("x-grok-client-version", xai_grok_version::VERSION)
                } else {
                    request
                };
                let request = if cfg!(test) {
