...
//!
//! Shared by keyboard navigation, mouse clicks, and action dispatch so every
//! link-opening path applies the same local-build safety policy.
...
pub fn is_safe_to_open(url: &str, filter: SchemeFilter) -> bool {
    ...
    if let Ok(parsed) = url::Url::parse(url) {
        // The clean build never opens a vendor account, billing, or hosted
        // product URL, even when one arrives through stale remote metadata or
        // a provider-generated message link. Keep ordinary user links and
        // local/file schemes subject only to the existing scheme filter.
        if !cfg!(test)
            && let Some(host) = parsed.host_str()
            && is_removed_vendor_host(host)
        {
            return false;
        }
...
fn is_removed_vendor_host(host: &str) -> bool {
...
///
/// Kept as a source-compatible URL helper for legacy callers. The clean
/// composition root does not use it for account or upgrade flows.
...
mod tests {
    ...
    #[test]
    fn removed_vendor_hosts_are_identified_exactly() {
        for host in [
            "x.ai",
            "api.x.ai",
            "grok.com",
            ...
            assert!(is_removed_vendor_host(host), "{host} should be blocked");
        }
        for host in ["example.com", "x.ai.evil.example", "notgrok.com"] {
            assert!(!is_removed_vendor_host(host), "{host} should remain usable");
        }
    }
