...
///
/// `xaicode wrap` runs a command inside a local PTY, scans its output for OSC 52
...
fn resolve_clipboard_route_with(ctx: &TerminalContext, no_osc52: bool) -> ClipboardRoute {
    ...
    // upstream wrap sink is capturing our output and will forward
...
impl ClipboardFeedback {
    ...
    fn message(self) -> &'static str {
        match self {
            ...
            Self::UnverifiedOscRemote | Self::UnverifiedOscContainer => {
                "Copy sent. If paste fails, use xaicode wrap or /minimal."
            }
...
/// clipboard over SSH (notably Apple Terminal without `xaicode wrap`); a failed
