...
impl TelemetryClient {
    pub fn from_config(
        ...
        // Keep this constructor for downstream source compatibility, but do
        // not materialize analytics endpoints, API keys, Mixpanel clients, or
        // account identifiers in the clean build.
        let _ = (
            config,
            mode,
            user_id,
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    // Product analytics transport is disabled in the clean build. Keep the
    // original implementation below for source/test compatibility, but make
    // the production path return before any HTTP or Mixpanel call.
...
/// profiles (`engage`).
pub fn sync_profile() {
    // Profile/people sync is a product analytics side effect. It is not
    // replaced by local logging and must stay a hard no-op even if an old
    // caller manages to construct a TelemetryClient directly.
}
...
pub fn init(
    ...
    // Product analytics is removed from the clean build. Keep the public
    // initializer so the original callers remain source-compatible, but do
    // not retain credentials, construct Mixpanel clients, or schedule HTTP.
    let _ = (
        config,
        mode,
        user_id,
