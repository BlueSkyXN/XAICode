...
pub fn init(cfg: Option<ExternalOtelConfig>) {
    // Never construct exporters or spawn an OTLP worker in the clean build.
    let _ = &cfg;
    return;
    ...
    #[allow(unreachable_code)]
    {
        ...
        if EXTERNAL.set(value).is_err() {
            tracing::debug!(
                "external otel: init called more than once; keeping first registration"
            );
        }
    }
}
...
pub fn emit<T: crate::events::TelemetryEvent>(data: &T) {
    // External OTLP/product telemetry is removed from the clean build.
    let _ = data;
    return;
