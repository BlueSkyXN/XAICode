...
impl Default for TelemetryConfig {
    fn default() -> Self {
        // Do not even materialize compile-time telemetry URLs/tokens in a
        // production clean build. The test configuration below still covers
        // the upstream build-env layering contract.
        if !cfg!(test) {
            return Self {
                enabled: Some(false),
                events_url: None,
...
impl TelemetryConfig {
    pub fn apply_env_overrides(&mut self) {
        // The clean build has no telemetry pipeline.  Do this before any
        // normalization or environment lookup so telemetry URLs, tokens, and
        // OTEL collector settings cannot be retained in memory from a local
        // config file or inherited process environment.  The test-only
        // resolution path remains available for the upstream compatibility
        // tests below.
        if !cfg!(test) {
