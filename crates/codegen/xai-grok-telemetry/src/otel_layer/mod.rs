...
{
    // The upstream layer could export authenticated OTLP spans to the hosted
    // proxy. Preserve the public constructor for downstream crates, but force
    // its exporter off in the clean build so even an explicit legacy call
    // cannot create a socket or attach vendor auth headers.
