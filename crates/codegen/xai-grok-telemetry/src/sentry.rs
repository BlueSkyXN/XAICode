...
pub fn init(config: Config) -> ClientInitGuard {
    // Error reporting is intentionally absent from the clean build. Return a
    // Sentry guard with no DSN so callers can retain their original lifetime
    // shape without creating a network client.
    let _ = &config;
...
pub fn flush_on_shutdown() {
    // No-op: the clean build never installs a Sentry client.
}
