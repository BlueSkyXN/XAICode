...
pub(crate) async fn require_bearer(auth: &SharedVoiceAuth) -> Result<String, VoiceError> {
    auth.bearer().await.ok_or_else(|| {
        VoiceError::Auth(
            "voice authentication is disabled in the clean local build; configure a provider if voice support is added"
                .into(),
        )
    })
}
...
///
/// Used by tests and by embedders that provide a short-lived bearer directly.
