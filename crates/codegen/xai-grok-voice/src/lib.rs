//! Optional voice input adapter for a coding agent and the
...
/// (the pager composition root), mirroring the pager's
...
pub fn maybe_run_capture_subprocess() -> Option<i32> {
    ...
    if !cfg!(test) {
        ...
        let _ = writeln!(
            std::io::stdout(),
            "ERR mic-capture helper disabled in the clean local build"
        );
        return Some(2);
    }
