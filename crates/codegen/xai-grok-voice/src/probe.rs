...
/// Capture mic audio and stream it to the configured STT provider, reporting
/// the transcript.
#[cfg(feature = "audio")]
pub async fn run_streaming_probe(opts: VoiceProbeOptions) -> Result<VoiceProbeReport, VoiceError> {
    if !cfg!(test) {
        ...
        return Err(VoiceError::Config(
            "voice probe is disabled in the clean local build".into(),
        ));
    }
...
pub fn format_probe_report(report: &VoiceProbeReport) -> String {
    let mut out = String::from("=== voice provider probe ===\n\n");
