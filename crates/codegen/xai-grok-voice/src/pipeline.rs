...
pub async fn run_voice_pipeline(
    ...
    // The upstream STT pipeline is a vendor-specific feature. The pager no
    // longer advertises it, and this compatibility entry point also fails
    // closed so a stale caller cannot open a microphone or WebSocket.
    let _ = (&config, &auth);
    let _ = event_tx
        .send(VoiceEvent::Error {
            message: "voice input is disabled in the clean local build".into(),
            hint: None,
        })
        .await;
