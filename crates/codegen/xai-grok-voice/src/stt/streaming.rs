...
/// Streaming STT over a caller-supplied provider endpoint.
    ...
    pub async fn connect(config: &VoiceConfig, bearer: &str) -> Result<Self, VoiceError> {
        if !cfg!(test) {
            ...
            return Err(VoiceError::Config(
                "voice/STT is disabled in the clean local build".into(),
            ));
        }
        ...
        // Optional request identity headers for a provider that needs them.
        // Skipped when empty or
...
mod tests {
    ...
    fn optional_header_inserted_when_present_skipped_when_empty() {
        let mut req = "wss://voice.invalid/v1/stt".into_client_request().unwrap();
        insert_optional_header(&mut req, "x-client-identifier", "xaicode");
        insert_optional_header(&mut req, "User-Agent", "");
        assert_eq!(req.headers().get("x-client-identifier").unwrap(), "xaicode");
        assert!(
