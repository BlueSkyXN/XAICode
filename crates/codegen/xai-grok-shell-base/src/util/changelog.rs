...
//!
//! The upstream project fetched markdown and JSON from a vendor CDN. The clean
//! build deliberately keeps only the cache reader so a startup or slash
//! command can never contact that control plane.
...
/// Non-routable compatibility base retained for the explicit test helper.
const CHANGELOG_BASE: &str = "http://127.0.0.1:0/disabled-changelog";
...
impl ChangelogManager {
    ...
    pub fn fetch(&self) -> Changelog {
        ...
        // (or OnceLock lag) still reads the live harness home. Never perform
        // the upstream network fetch in the clean local build.
