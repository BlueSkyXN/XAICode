//! Shared upload utilities for optional remote object storage.
//!
//! The clean local build keeps these types and mock seams for source
//! compatibility, but production entry points fail closed before any cloud,
//! proxy, or S3 request is created.
...
pub async fn upload_bytes<C: StorageConfig>(
    ...
    if !cfg!(test) {
        let _ = (config, object_path, content, content_type);
        anyhow::bail!("remote object storage is disabled in the clean local build");
    }
...
pub async fn upload_file<C: StorageConfig>(
    ...
    if !cfg!(test) {
        let _ = (config, object_path, file_path, content_type);
        anyhow::bail!("remote object storage is disabled in the clean local build");
    }
    match config.upload_method() {
        UploadMethod::Direct {
...
{
    if !cfg!(test) {
        let _ = (config, object_path, content_type);
        drop(reader);
        anyhow::bail!("remote object storage is disabled in the clean local build");
    }
    match config.upload_method() {
