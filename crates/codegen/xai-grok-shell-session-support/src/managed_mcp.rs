...
async fn get_authenticated_json<T: serde::de::DeserializeOwned>(
    ...
    if !cfg!(test) {
        // This function is retained for the upstream test seam only.  The
        // clean build has no hosted managed-MCP transport.
        return Err(ManagedMcpFetchError::NoAuth);
    }
...
pub async fn get_or_fetch(
    ...
    if !cfg!(test) {
        let _ = (handle, proxy_url, auth_key, refresh_ctx);
        return vec![];
    }
...
pub async fn get_or_fetch_gateway_tool_catalog(
    ...
    if !cfg!(test) {
        let _ = (handle, proxy_url, auth_key);
        return None;
    }
