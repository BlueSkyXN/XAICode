...
/// Construct an [`OriginClientInfo`] from the generic client env vars.
/// Upstream `GROK_CLIENT_*` names are read only by compatibility tests; a
/// production clean build never adopts a vendor-specific process identity.
...
pub fn process_user_agent_string() -> String {
    ...
    UserAgent {
        origin,
        agent_product: DEFAULT_CLIENT_IDENTIFIER,
        agent_version,
...
/// Process-level client identifier (`CODING_AGENT_CLIENT_NAME`, default
/// `"xaicode"`). The old env name is retained only for test fixtures.
pub fn process_client_identifier() -> String {
    std::env::var("CODING_AGENT_CLIENT_NAME")
        .ok()
        .or_else(|| {
            if cfg!(test) {
                std::env::var("GROK_CLIENT_NAME").ok()
...
mod tests {
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("grok-desktop/1.2.3 xaicode/"));
        ...
        assert!(without_version.starts_with("grok-web xaicode/"));
