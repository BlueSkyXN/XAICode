...
async fn connect_and_auth() -> acp::ClientSideConnection {
    ...
    let method = init
        ...
        .expect("api_key auth method not advertised");
    tokio::time::timeout(
        RPC_TIMEOUT,
...
fn session_churn_returns_registry_snapshot_to_baseline() {
    ...
    unsafe {
        ...
        std::env::set_var("CODING_AGENT_API_BASE_URL", server.url());
        std::env::set_var("CODING_AGENT_MODELS_BASE_URL", server.url());
        std::env::set_var("CODING_AGENT_API_KEY", "test-key-for-ci");
