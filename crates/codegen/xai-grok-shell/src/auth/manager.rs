...
impl AuthManager {
    ...
    ///
    /// Unlike [`Self::new`], this path never reads `auth.json`, `GROK_AUTH`, or
    /// any cached session token. Provider API keys are supplied through the
    /// generic model/provider configuration and are kept in the agent's
    /// process-local credential path.
    ...
    pub fn new(grok_home: &Path, grok_com_config: GrokComConfig) -> Self {
        // Compatibility constructor: all runtime construction is account-free
        // in the clean build. The historical auth.json/OIDC loader below is
        // unreachable reference code and must not be reactivated by a stale
        // caller.
        if !cfg!(test) {
            return Self::new_local(grok_home, grok_com_config);
        }
    ...
    pub(crate) fn clear(&self) -> std::io::Result<()> {
        // A clean-build logout is an in-memory operation. Never remove or
        // rewrite a legacy hosted auth.json that may have been left behind by
    ...
    pub(crate) async fn update(self: &Arc<Self>, auth: GrokAuth) -> std::io::Result<GrokAuth> {
        // Session/OIDC persistence belongs to the removed hosted account
        // product. Keep the in-memory seam for compatibility, but do not
        // create or mutate auth.json in a production clean build.
        if !cfg!(test) {
    ...
    fn read_disk_auth_silent(&self) -> Option<GrokAuth> {
        // Keep the legacy reader for unit tests that exercise token rotation,
        // but never adopt a pre-existing hosted auth.json in production.
        if !cfg!(test) {
            return None;
        }
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<GrokAuth>, DiskAuthState) {
        // The clean build is account-free. Preserve the state-machine contract
        // without touching the legacy hosted credential file at runtime.
        if !cfg!(test) {
    ...
    pub fn configure_refresher(
        ...
        if !cfg!(test) {
            let _ = (&auth_provider_command, &diagnostic_uploader);
            return false;
        }
    ...
    pub(crate) fn is_devbox_environment(&self) -> bool {
        // Devbox recovery mints hosted OIDC credentials and is not part of the
        // local coding-agent build. Keep the real probe only for legacy tests.
        if !cfg!(test) {
            return false;
        }
        #[cfg(test)]
...
/// Tools + pager bearer. Static: env → process model key.
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    ...
    let disk_key = cfg!(test).then(|| am.cached_disk_api_key()).flatten();
