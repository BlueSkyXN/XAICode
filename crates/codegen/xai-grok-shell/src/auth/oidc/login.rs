...
fn callback_response(result: &CallbackResult) -> (StatusCode, Html<String>) {
    let (title, message) = match result {
        Ok(_) => (
            "Signed in",
            "You can close this window and return to xaicode.",
        ),
...
pub async fn run_login_flow(
    ...
    if !cfg!(test) {
        let _ = (config, auth_manager, channels);
        anyhow::bail!("OIDC account authentication is disabled in the clean local build.");
    }
    let oidc = config
        .oidc
        .as_ref()
...
pub async fn run_login_flow_with_config(
    ...
    if !cfg!(test) {
        let _ = (oidc, auth_manager, channels);
        anyhow::bail!("OIDC account authentication is disabled in the clean local build.");
    }
