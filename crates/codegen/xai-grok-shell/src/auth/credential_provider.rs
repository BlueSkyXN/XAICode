...
impl AuthCredentialProvider for ShellAuthCredentialProvider {
    ...
    fn needs_token_auth_header(&self) -> bool {
        // The upstream storage client uses this bit to add the vendor-only
        // `X-XAI-Token-Auth` marker.  It is retained for compatibility tests,
        // but must stay off in a production clean build.
        cfg!(test) && self.static_credentials.deployment_key.is_none()
    }
}
...
///   - `user_token` → sends a normal Bearer credential
///   - `deployment_key` → sends a normal Bearer credential
...
impl AuthCredentialProvider for OtelAuthCredentialProvider {
    ...
    fn needs_token_auth_header(&self) -> bool {
        // See the corresponding implementation on
        // `ShellAuthCredentialProvider`: this flag controls a vendor-specific
        // header in the upstream OTel exporter.
        cfg!(test) && self.deployment_key.load().is_none()
    }
}
...
pub fn build_default_otel_layer_config() -> xai_grok_telemetry::otel_layer::OtelLayerConfig {
    // Keep the constructor for source compatibility, but make the returned
    // layer inert: hosted OTLP export and auth.json bootstrap are removed from
    // the clean build.
    let grok_com_config = crate::auth::GrokComConfig::default();
    let exporter = xai_grok_telemetry::otel_layer::OtelExporterConfig {
        traces_url: "http://127.0.0.1:0/disabled-otlp".to_owned(),
    ...
    let bootstrap = Arc::new(AuthManager::new_local(&grok_home, grok_com_config));
