...
pub enum AuthError {
    ...
    #[error(
        "Local provider credentials are unavailable. Set CODING_AGENT_API_KEY or OPENAI_API_KEY."
    )]
    TokenExpiredNoRefresh,

    /// Server rejected the token (401) with no recovery path.
    #[error("The provider rejected the request. Check CODING_AGENT_API_KEY or OPENAI_API_KEY.")]
    ...
    #[error("{message} Configure the required local provider credentials.")]
    PinnedTeamMismatch { message: String },

    /// Cached API-key session rejected because API-key auth is disabled.
    #[error("API-key authentication is disabled by the local configuration.")]
    ApiKeyAuthDisabled,
    ...
    pub(crate) fn user_message(self) -> Cow<'static, str> {
        match self {
            Self::RefreshTokenRejected => {
                "The provider session is unavailable. Configure a local provider API key.".into()
            }
            Self::ClientRejected => {
                "Provider authentication is temporarily unavailable. Check your local API key."
                    .into()
            }
            Self::ProviderInteractiveRequired => {
                "Interactive provider authentication is disabled; configure a local API key.".into()
            }
            Self::Other => {
                "Provider authentication could not be refreshed. Check your local API key.".into()
            }
        }
    }
}
