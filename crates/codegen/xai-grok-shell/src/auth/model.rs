...
/// Local compatibility scope; hosted legacy auth files are not read.
pub(super) const LEGACY_SCOPE: &str = "coding-agent::local";
...
pub struct GrokAuth {
    ...
    /// Issuer URL retained for deserializing old credentials. The clean build
    /// never uses it to select an endpoint or refresh an account token.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl GrokAuth {
    ...
    /// `true` when a hosted vendor account credential is recognized. The clean
    /// build recognizes none; this compatibility predicate always fails closed.
    ///
