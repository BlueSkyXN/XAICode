...
pub(crate) enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Configure CODING_AGENT_API_KEY or OPENAI_API_KEY instead."
    )]
    NotEnabled,
}
...
pub(crate) async fn request_device_code(
    ...
    if !cfg!(test) {
        let _ = (issuer, client_id, scopes, surface);
...
pub(crate) async fn complete_device_code_login(
    ...
    if !cfg!(test) {
        let _ = (issuer, client_id, &device_code, auth_manager, surface);
        anyhow::bail!("Device-code account authentication is disabled in the clean local build.");
    }
    let client = crate::http::shared_client();
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!(
                "Device-code authentication is disabled in the clean local build. Configure a local provider API key."
            );
        }
...
pub(crate) async fn run_device_code_login_channels(
    ...
    if !cfg!(test) {
        let _ = (issuer, client_id, scopes, auth_manager, channels);
        anyhow::bail!("Device-code account authentication is disabled in the clean local build.");
    }
