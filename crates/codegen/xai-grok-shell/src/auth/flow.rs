...
async fn run_external_auth_provider(
    ...
    // Keep the compatibility implementation available to unit tests, but do
    // not allow a production build to execute an arbitrary account-login
    // helper.  The clean agent accepts provider API keys only; external/OIDC
    // account authentication is intentionally not part of that contract.
    if !cfg!(test) {
        let _ = (command, auth_manager, over_stale_credential, on_stderr);
        anyhow::bail!("external account authentication is disabled in the clean local build");
    }
...
pub(crate) async fn run_auth_flow_with_stderr_bridge(
    ...
    if !cfg!(test) {
        ...
        anyhow::bail!("Interactive account authentication is disabled in the clean local build.");
    }
...
async fn run_auth_flow_steps(
    ...
    tracing::error!(
        "auth: no local provider credentials or supported API-key configuration available"
    );
    anyhow::bail!(
        "No local provider credentials are configured. Set CODING_AGENT_API_KEY or OPENAI_API_KEY."
    )
}
...
pub async fn ensure_authenticated(
    ...
    if !cfg!(test) {
        let _ = (grok_com_config, reauth, message_prefix);
        anyhow::bail!("Interactive account authentication is disabled in the clean local build.");
    }
...
pub async fn ensure_authenticated_with_override(
    ...
    if !cfg!(test) {
        let _ = (grok_com_config, reauth, message_prefix, login_override);
        anyhow::bail!("Interactive account authentication is disabled in the clean local build.");
    }
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    if !cfg!(test) {
        let _ = (grok_com_config, has_noninteractive_auth, message_prefix);
        return Ok(None);
    }
...
pub async fn run_cli_login(
    ...
    if !cfg!(test) {
        ...
        anyhow::bail!("Account login is disabled in the clean local build.");
    }
...
async fn run_cli_login_steps(
    ...
    let authenticated = if cli_should_use_device(&config.grok_com_config, login_override).await {
        if config.grok_com_config.oauth2.is_none() {
            ...
            anyhow::bail!(
                "Account sign-in is disabled in the clean local build. Set CODING_AGENT_API_KEY or OPENAI_API_KEY."
            );
        }
...
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    if !cfg!(test) {
        let _ = config;
        anyhow::bail!("Account logout is disabled in the clean local build.");
    }
    let grok_home = grok_home::grok_home();
    ...
    if !result.was_logged_in {
        ...
        if result.api_key_still_set {
            eprintln!(
                "A local provider API key is set in the environment and will be used for authentication."
            );
        }
        return Ok(());
    }
    ...
    if result.api_key_still_set {
        eprintln!("A local provider API key is still set and will be used for authentication.");
    }
    Ok(())
}
