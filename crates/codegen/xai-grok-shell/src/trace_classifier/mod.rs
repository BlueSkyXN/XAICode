...
/// Resolve the API key from `--api-key` and generic provider environment
/// variables. The offline classifier never reads the hosted auth store or
/// starts an account refresh flow.
pub async fn resolve_api_key(explicit: Option<&str>, _grok_home: &Path) -> Result<String> {
    if let Some(k) = explicit.map(str::trim).filter(|s| !s.is_empty()) {
        return Ok(k.to_owned());
    }
    for name in ["CODING_AGENT_API_KEY", "OPENAI_API_KEY"] {
    ...
    Err(anyhow!(
        "no API key: pass --api-key, set CODING_AGENT_API_KEY or OPENAI_API_KEY, \
         or configure api_key/env_key in config.toml."
    ))
}
...
mod tests {
    ...
    async fn resolve_api_key_precedence() {
        ...
        unsafe { std::env::set_var("CODING_AGENT_API_KEY", "from-env") };
        assert_eq!(
        ...
        unsafe { std::env::set_var("CODING_AGENT_API_KEY", "   ") };
        assert!(
            resolve_api_key(None, grok_home).await.is_err(),
    ...
    #[tokio::test]
    #[serial_test::serial]
    #[ignore = "hosted auth.json fallback was removed"]
    ...
    async fn resolve_api_key_errors_when_all_three_sources_missing() {
        ...
        assert!(
            msg.contains("--api-key")
                && msg.contains("CODING_AGENT_API_KEY")
                && msg.contains("OPENAI_API_KEY")
                && msg.contains("config.toml"),
            "error names local sources: {msg}",
        );
    }
    ...
    #[tokio::test]
    #[serial_test::serial]
    #[ignore = "hosted OIDC refresh was removed"]
    ...
    /// Env vars cleared by [`with_env_isolated`] / inline test teardown.
    const ISOLATED_ENV_KEYS: &[&str] = &["CODING_AGENT_API_KEY", "OPENAI_API_KEY"];
