...
pub enum ConvError {
    #[error("hosted conversation history is disabled in the clean local build")]
    Disabled,
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        // Conversation history belongs to the hosted chat product. Keep the
        // type available for source compatibility, but ignore legacy URL env
        // overrides so constructing it cannot create a remote client.
    ...
    pub async fn update_conversation(
        ...
        let _ = (self, conversation_id, body);
        return Err(ConvError::Disabled);
