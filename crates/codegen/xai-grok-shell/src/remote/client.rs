...
pub async fn fetch_subagent_bundle(
    ...
    return Err(BackendError::Auth(
        "hosted subagent bundles are disabled in the clean local build".to_owned(),
    ));
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            // Ignore GROK_CODE_BACKEND_URL from older installations. This
            // client remains in the crate graph for local-session source
            // compatibility, but the clean build never talks to the hosted
            // share/sync backend.
            base_url: GROK_CODE_BACKEND_URL.to_string(),
            auth_manager: None,
        }
    }
    ...
    async fn send_with_auth(
        ...
        return Err(BackendError::Auth(
            "hosted session sharing/sync is disabled in the clean local build".to_owned(),
        ));
        #[allow(unreachable_code)]
...
pub(crate) fn fetch_models_blocking(
    ...
    Err(BackendError::Auth(
        "remote model catalogs are disabled in the clean local build".to_owned(),
    ))
    /*
    let client = crate::http::shared_startup_blocking_client();
    ...
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = crate::agent::auth_method::read_xai_api_key_env()
                ...
                .map_err(|_| {
                    BackendError::Auth(
                        "No API key for the custom models endpoint. Set CODING_AGENT_API_KEY or OPENAI_API_KEY.".into(),
                    )
                })?;
...
// Hosted backend tests exercise endpoints that are not part of the clean
// local build. Keep them in the source tree for upstream reference without
// compiling them into the provider-neutral test suite.
#[cfg(any())]
mod tests {
    use super::*;
    use axum::{
