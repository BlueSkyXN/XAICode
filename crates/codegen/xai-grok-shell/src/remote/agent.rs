...
const CLEAN_LOCAL_SANDBOX_URL: &str = "http://127.0.0.1:0/disabled-sandbox";
...
impl SandboxClient {
    pub fn new(base_url: impl Into<String>, auth_manager: Arc<AuthManager>) -> Self {
        ...
        Self {
            ...
            base_url: CLEAN_LOCAL_SANDBOX_URL.to_owned(),
            auth_manager,
        }
    }
    ...
    async fn auth_headers(
        ...
        // Cloud sandbox lifecycle is a hosted product feature. Keep the
        // request-builder seam for source compatibility, but never resolve
        // account credentials or open a network connection in this build.
        let _ = (&self, &builder);
        return Err(anyhow::anyhow!(
            "hosted sandbox management is disabled in the clean local build"
        ));
