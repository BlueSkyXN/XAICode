...
const GROK_WEB_URL: &str = "http://127.0.0.1:0/disabled-web";
...
pub enum ChatModelsError {
    #[error("hosted chat model catalog is disabled in the clean local build")]
    Disabled,
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        // Chat-product modes are intentionally not configurable in the clean
        // build. Ignore stale GROK_* URL overrides instead of allowing an old
        // installation to re-enable a hosted request path.
        let base_url = GROK_WEB_URL.to_string();
        Self {
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        // Fail closed even when a downstream caller bypasses the chat-mode
        // manager and invokes the transport directly.
        let _ = (self, locale);
        return Err(ChatModelsError::Disabled);
