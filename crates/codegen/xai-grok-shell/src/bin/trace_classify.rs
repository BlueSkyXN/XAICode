...
//!       [--api-key <key> | $CODING_AGENT_API_KEY] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use xai_grok_shell::trace_classifier::validate_min_confidence;
...
struct Cli {
    ...
    /// Provider API key. The clean build never reads a hosted auth file.
    #[arg(long)]
    ...
    /// Deprecated compatibility argument; local provider authentication never
    /// reads a hosted auth store.
    #[arg(long = "grok-home", hide = true)]
...
async fn main() -> anyhow::Result<()> {
    let cli = Cli::parse();
    let _ = cli;
    anyhow::bail!("the hosted trace-classification CLI is disabled in the clean local build")
}
...
mod tests {
    ...
    fn cli_parses_minimal_args() {
        ...
        assert_eq!(cli.api_base_url, "http://127.0.0.1:8000/v1");
    ...
    fn cli_defaults_match_documented_values() {
        ...
        assert_eq!(by_id("model"), vec!["local-model"]);
        assert_eq!(by_id("api_base_url"), vec!["http://127.0.0.1:8000/v1"]);
