            ...
            Self::NotFound { name } => {
                write!(
                    ...
                    "Plugin \"{name}\" not found.\n\
                     Run `xaicode plugin list` to see installed plugins."
                )
            }
            Self::NeedsConfirm {
                name,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `xaicode plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `xaicode plugin install ./{name}`, or add a \
                     source with `xaicode plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `xaicode plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `xaicode plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
/// Like [`load_marketplace_sources`] but drops removed vendor sources and git
/// sources blocked by the managed `marketplace_allowlist`. Install paths must
/// use this so policy cannot be bypassed.
...
fn filter_sources_by_allowlist(
    ...
    sources.retain(|source| match &source.kind {
        SourceKind::Git { url, .. }
            if xai_grok_plugin_marketplace::is_blocked_vendor_source_url(url) =>
        {
            tracing::debug!(name = %source.name, url, "removed vendor marketplace source ignored");
            false
        }
        _ => true,
    });
...
mod tests {
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("xaicode plugin install ./sentry"), "{msg}");
        assert!(msg.contains("xaicode plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        ...
        assert!(
            msg.contains("xaicode plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
