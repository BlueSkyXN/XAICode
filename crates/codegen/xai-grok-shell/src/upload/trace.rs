    ...
    if !cfg!(test) {
        if let Some(filename) = gcs_path.rsplit('/').next() {
            super::manifest::skip_artifact(
                &ctx.artifact_tracker,
                filename,
                "trace_upload_disabled",
            );
        }
        let _ = (content, content_type, artifact_name, deadline);
        return Ok(());
    }
...
pub(crate) async fn upload_trace_artifact(
    ...
    if !cfg!(test) {
        ...
        let _ = (content, content_type, artifact_name);
        return;
    }
