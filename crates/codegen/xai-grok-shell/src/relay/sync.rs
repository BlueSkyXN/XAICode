...
/// Build a local diagnostic session URI for compatibility callers.
/// Hosted session sharing is removed from the clean build, so this function
/// deliberately never produces a web URL.
...
impl RelaySync {
    ...
    pub fn new(
        ...
        // Session sharing is a hosted product feature. Keep the public type
        // constructible for the upstream crate graph, but do not spawn the
        // relay task or open a WebSocket in the clean build.
        let _ = (config, agent_type, session_dir, status_cb);
