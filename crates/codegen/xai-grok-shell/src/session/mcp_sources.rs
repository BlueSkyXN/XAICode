//! Shell-side MCP merge for local/plugin/compat sources plus admitted clients.
//!
...
/// Normalize local HTTP/SSE URLs for merge deduplication.
...
pub(crate) fn merge_mcp_servers(
    ...
    merge_mcp_servers_with_policy(client_mcp_servers, cwd, plugin_registry, compat)
        .into_iter()
...
pub(crate) fn merge_and_send_mcp_update(
    ...
    let merged = merge_mcp_servers(initial_client_mcp_servers, cwd, plugin_registry, compat);
...
pub(crate) fn merge_mcp_servers_with_policy(
    client_mcp_servers: Vec<acp::McpServer>,
    cwd: &std::path::Path,
    plugin_registry: Option<&xai_grok_agent::plugins::PluginRegistry>,
    compat: &xai_grok_tools::types::compat::CompatConfig,
    ...
    let mut servers: HashMap<String, acp::McpServer> =
        merge_mcp_servers_sourced(cwd, plugin_registry, compat)
            .into_iter()
...
/// `merge_mcp_servers` then drops every tagged server. Split out from
/// `merge_mcp_servers_with_policy` so the deny/allow enforcement
...
pub(crate) fn merge_mcp_servers_sourced(
    ...
    let _mcp_merge_timer = crate::instrumentation::timer("mcp_merge_sources");
...
/// [`merge_mcp_servers_with_policy`]. Does **not** include client
...
mod tests {
    ...
    fn client_provided_servers_survive_merge() {
        ...
        let merged = merge_mcp_servers(client, cwd.path(), None, &compat);
        assert!(
...
"#,
    ...
    fn admitted_seed_stays_blocked_after_vendor_disk_vanishes() {
        ...
        let merged = merge_mcp_servers(admitted, cwd.path(), None, &compat);
        assert!(
            !merged
                .iter()
                .any(|s| mcp_server_name(s) == "killswitch-cache"),
    ...
    fn client_cursor_http_dropped_by_normalized_url_when_mcps_disabled() {
        ...
        let merged = merge_mcp_servers(vec![matching, other], cwd.path(), None, &compat);
        assert!(
            !merged
                .iter()
    ...
    fn merge_drops_denied_server_and_classifies_as_denylist() {
        ...
        // The public `merge_mcp_servers` drop predicate removes exactly
        // the denied server.
    ...
    /// An explicitly configured `grok_com_*` name is still an ordinary local
    /// server name. The policy seam must accept the exact configured spelling;
    /// no hosted connector catalog is involved in the merge.
    #[test]
    fn explicit_grok_com_name_is_policy_addressable_as_local() {
        use xai_grok_workspace::permission::resolution::{AllowedMcpServer, McpServerAllowlist};

        let name = "grok_com_slack";
        let server = acp::McpServer::Http(
            acp::McpServerHttp::new(name, "https://example.com/mcp").headers(vec![]),
        );
        let deny = McpServerAllowlist::new(
            vec![],
            vec![AllowedMcpServer::Name { name: name.into() }],
            Some(std::path::PathBuf::from("/test/managed-settings.json")),
        );
        let tagged = apply_mcp_server_policy(
            vec![server.clone()],
        ...
        let allow = McpServerAllowlist::new(
            vec![AllowedMcpServer::Name { name: name.into() }],
            vec![],
            Some(std::path::PathBuf::from("/test/managed-settings.json")),
        );
        let tagged =
            apply_mcp_server_policy(vec![server], &std::collections::HashSet::new(), &allow);
        assert!(tagged[0].disabled_reason.is_none());
    }
...
"#,
        ...
        let compat = xai_grok_tools::types::compat::CompatConfig::default();
        let merged = merge_mcp_servers(vec![], cwd.path(), None, &compat);
        assert!(
    ...
    fn untrusted_workspace_drops_project_mcp_servers() {
        ...
        let merged = merge_mcp_servers(client, untrusted.path(), None, &compat);
        assert!(
        ...
        let merged = merge_mcp_servers(vec![], trusted.path(), None, &compat);
        assert!(
            merged.iter().any(|s| mcp_server_name(s) == "projsrv"),
    ...
    fn plugin_server_deduped_across_file_and_inline() {
        ...
        let cwd = tempfile::tempdir().unwrap();
        let compat = xai_grok_tools::types::compat::CompatConfig::default();
        let sourced = merge_mcp_servers_sourced(cwd.path(), Some(&registry), &compat);
