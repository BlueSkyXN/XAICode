...
pub async fn maybe_upload_index(
    ...
    // The clean build never synchronizes session data with hosted storage.
    // Keep the implementation below for source/test compatibility, but make
    // the production entry point an unconditional local-only no-op.
    if !cfg!(test) {
        let _ = (db_path, config, gcs_config, auth_manager);
        return;
    }
    if !config.enabled {
        return;
...
pub async fn maybe_download_index(
    ...
    // Do not probe or download from GCS in a production clean build.
    if !cfg!(test) {
        let _ = (db_path, config, gcs_config, auth_manager);
        return false;
    }
