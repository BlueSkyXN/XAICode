...
/// Regression for the cancel-during-long-reasoning persistence gap:
...
/// `CopyFile` to snapshot the session directory.
///
        ...
        .run_until(async {
            ...
            assert_eq!(
                ...
                "Cancel/CopyFile handler flush must persist the buffered \
                     reasoning chunk before the session snapshot reads \
                     updates.jsonl"
            );
            drop(actor);
        })
...
/// budget-spent accept — no tally or counters; the signals stay warn-only on
...
/// A recovered turn preserves the doom-loop tally, retry signals, and accepted
/// after-budget accounting across the real sampler event path.
#[tokio::test(flavor = "current_thread")]
async fn doom_loop_recovery_tracks_counters_and_signals() {
