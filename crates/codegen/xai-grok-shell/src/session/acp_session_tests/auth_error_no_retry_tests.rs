...
async fn legacy_auth_hint_on_404_model_not_found() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                msg.contains("legacy credential"),
                "hint must mention the legacy credential, got: {msg}"
            );
            assert!(
                msg.contains("CODING_AGENT_API_KEY") || msg.contains("OPENAI_API_KEY"),
                "hint must mention a local provider API key, got: {msg}"
            );
            assert!(
