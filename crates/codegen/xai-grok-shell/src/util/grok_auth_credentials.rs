...
use std::sync::Arc;
/// Credentials for authenticating with a configured model/backend service.
///
...
///
/// A deployment key and a user token are both represented as bearer
/// credentials.  The clean build deliberately does not add the former
/// vendor's `X-XAI-Token-Auth` marker; callers can use ordinary provider
/// authentication against their configured local or third-party endpoint.
...
impl GrokAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your deployment key is invalid or expired. Please contact an administrator."
        } else if self.user_token.is_some() {
            "Your API token is invalid or expired. Check the configured provider credentials."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        let builder = if let Some(ref key) = self.deployment_key {
            ...
            // Keep the legacy wire shape available to unit tests that exercise
            // the upstream compatibility layer, but never emit it from a
