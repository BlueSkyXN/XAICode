...
pub fn resolve_remote_fetch_enabled() -> bool {
    // A local Coding Agent never contacts the hosted settings/model catalog.
    // Keep the policy helpers below for source compatibility with the upstream
    // configuration/tests, but make the runtime gate unconditional so a stale
    // managed file or environment cannot re-arm the network path.
    false
}
