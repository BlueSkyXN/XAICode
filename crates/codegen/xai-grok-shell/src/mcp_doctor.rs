//! `xaicode mcp doctor` -- runtime health check for MCP servers.
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `xaicode mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `xaicode mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `xaicode mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `xaicode mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
