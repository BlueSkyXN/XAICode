...
pub(crate) fn has_active_team_auth() -> bool {
    // Managed/team configuration is not part of the local Coding Agent.
    false
}
...
pub fn clear_orphan() {
    // Keep the compatibility entry point, but never inspect auth.json or
    // mutate managed-policy files in the clean local build.
    return;
...
pub fn spawn_sync(cancel: tokio_util::sync::CancellationToken) {
    // Hosted managed-config polling is removed. Retain the function so older
    // callers still compile without starting a background task.
    let _ = cancel;
    return;
...
pub fn resolve_deployment_key() -> Option<String> {
    // Deployment keys are an xAI-managed-config credential and are ignored.
    return None;
...
pub fn is_fetch_enabled() -> bool {
    // No hosted policy/config endpoint is reachable from the local build.
    return false;
...
pub async fn ensure_managed_policy_present(
    ...
    // Compatibility tests can exercise the original managed-policy flow;
    // production is stopped above before any credential or disk access.
    ...
    // Tests retain the upstream classification before exercising the fetch gate.
    if !is_fetch_enabled() {
        return;
    }
...
pub fn managed_policy_gate() -> Result<(), String> {
    // Hosted account policy gates do not exist in the local build.
    return Ok(());
...
pub async fn run_setup() -> SetupOutcome {
    // `setup` is retained as a source-compatible command surface only.
