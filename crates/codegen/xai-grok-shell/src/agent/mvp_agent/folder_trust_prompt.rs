...
/// `broadcast_plugin_registry_to_sessions`: `merge_mcp_servers`
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // MCP: `merge_mcp_servers` re-reads disk + runs
        ...
        let _ = crate::session::mcp_sources::merge_and_send_mcp_update(
            &target.cmd_tx,
            session_cwd,
