...
impl MvpAgent {
    ...
    fn feedback_credentials(
        ...
        // Feedback is a hosted product service. Keep the helper signature for
        // the session builder, but never construct a client or send a request.
        let _ = self;
        None
    }
    ...
    async fn fetch_settings(
        ...
        // Remote settings are an upstream hosted-product control plane.  The
        // clean build is intentionally local-only, so even a direct call from
        // a legacy auth/session path must not issue a network request.
        let _ = (self, auth);
    ...
    pub(super) fn spawn_settings_reapply(&self) {
        // The clean build has no hosted settings endpoint. Keep this seam
        // available to compatibility tests, but never even spawn an auth or
        // settings task in a production build.
        if !cfg!(test) {
            return;
        }
        let agent_ref = LocalRef::new(self);
    ...
    pub(super) fn spawn_post_auth_settings(&self, auth: crate::auth::GrokAuth) {
        ...
        // that could touch account state or remote settings in production.
        if !cfg!(test) {
            return;
        }
        let agent_ref = LocalRef::new(self);
    ...
    pub fn with_models(
        ...
        if relay_sync_enabled {
            tracing::info!("[xaicode] Relay sync: ENABLED");
        } else if tui_mode && relay_config_enabled && !has_xai_auth {
            tracing::info!(
                "[xaicode] Relay sync: DISABLED (hosted relay is unavailable in the clean local build)"
            );
