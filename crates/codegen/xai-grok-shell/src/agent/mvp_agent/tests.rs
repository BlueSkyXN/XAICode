...
/// the next value (current + 1) so the counter survives restarts.
#[test]
...
/// Session-local MCP changes only rebuild the search index; the index has no
/// hosted connector catalog dependency.
#[tokio::test(flavor = "current_thread")]
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    agent.session_registry.set_turn_number(&sid, 3);
            ...
            match tokio::time::timeout(std::time::Duration::from_secs(2), cmd_rx.recv()).await {
                Ok(Some(TestSessionCommand::UpdateMcpServers { mcp_servers, .. })) => {
                    saw_project_mcp |= mcp_servers
                        .iter()
                        .any(|s| crate::session::mcp_sources::mcp_server_name(s) == "projsrv");
                }
        ...
        match tokio::time::timeout(std::time::Duration::from_secs(2), cmd_rx.recv()).await {
            Ok(Some(TestSessionCommand::UpdateMcpServers { mcp_servers, .. })) => {
                ...
                out.mcp_names = mcp_servers
                    .iter()
                    .map(|s| crate::session::mcp_sources::mcp_server_name(s).to_string())
                    .collect();
            }
            Ok(Some(TestSessionCommand::ReloadPlugins { .. })) => {
