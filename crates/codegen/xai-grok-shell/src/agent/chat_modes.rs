...
impl ChatModesManager {
    ...
    pub(crate) async fn model_state(&self) -> acp::SessionModelState {
        // The upstream chat-mode catalog is a hosted Grok product endpoint.
        // Keep the mapping/cache types for compatibility, but never resolve a
        // stale `--chat`/ACP request into a network fetch.
        let _ = self;
        return empty_state();
    ...
    pub(crate) fn warm_in_background(&self) {
        // Hosted chat-mode prefetch is disabled in the clean build.
        let _ = self;
        return;
