...
impl RelayConfig {
    ...
    pub(crate) fn for_session(
        ...
        if !cfg!(test) {
            let _ = (session, ctx, alpha_test_key, auth_manager);
            return None;
        }
...
pub(crate) fn spawn_relay_connection_with_callback(
    ...
    if !cfg!(test) {
        let _ = (config, to_agent_tx, agent_to_ws_rx, on_first_connect);
        cancel.cancel();
        return (agent_to_ws_tx, RelayHandle { cancel });
    }
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!(
            "Hosted relay authentication is disabled in the clean local build. Configure a local provider API key."
        );
        cancel.cancel();
        return false;
    };
