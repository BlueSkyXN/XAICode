...
mod clean_tests {
    ...
    #[test]
    fn only_generic_api_key_is_advertised() {
        let built = build_auth_methods(AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: true,
            has_enterprise_oidc: true,
            enterprise_oidc_issuer: Some("https://idp.example.invalid"),
            login_label: Some("ignored"),
            has_auth_provider_command: true,
            preferred_method: Some(PreferredAuthMethod::Oidc),
        });
        assert_eq!(built.methods.len(), 1);
        assert_eq!(built.methods[0].id(), &acp::AuthMethodId::new("api_key"));
        assert_eq!(
            built.default_auth_method_id,
            Some(acp::AuthMethodId::new("api_key"))
        );
    }

    #[test]
    fn no_key_advertises_no_interactive_fallback() {
        let built = build_auth_methods(AuthMethodsBuildInputs {
            has_external_api_key: false,
            has_cached_token: true,
            has_enterprise_oidc: true,
