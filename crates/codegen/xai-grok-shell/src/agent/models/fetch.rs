...
fn prefetch_models_blocking_gated(
    ...
    // A local models cache is still valid input for xaicode. Only the
    // network fetch below is gated; this lets users keep offline model catalogs
    // without re-enabling the hosted control plane.
...
fn resolve_startup_endpoints() -> config::EndpointsConfig {
    // Startup model discovery is local-only; never hydrate a deployment key
    // from the hosted managed-config subsystem.
...
fn start_early_prefetch_with_auth_gated(
    ...
    if !crate::util::config::resolve_remote_fetch_enabled() {
        tracing::info!("startup model/settings prefetch skipped: remote fetch disabled");
        return None;
    }
...
fn start_early_prefetch_impl(
    ...
    let auth = AuthManager::new_local(&grok_home, grok_com_config.unwrap_or_default()).current();
