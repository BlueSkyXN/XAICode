...
pub const DEFAULT_AGENT_TYPE: &str = "xaicode";
...
/// Default local OpenAI-compatible provider endpoint. Users should override it
/// with `CODING_AGENT_API_BASE_URL` or a model-provider `base_url`.
pub const CLI_CHAT_PROXY_BASE_URL_DEFAULT: &str = "http://127.0.0.1:8000/v1";
/// Compatibility field for the original inference endpoint; it now resolves
/// to the same generic local provider default and never to an xAI host.
pub const XAI_API_BASE_URL_DEFAULT: &str = CLI_CHAT_PROXY_BASE_URL_DEFAULT;
...
impl EndpointsConfig {
    /// Return `true` for the upstream vendor hosts that the clean build must
    /// never contact. This is deliberately host-oriented rather than a blanket
    /// ban on custom providers: arbitrary OpenAI-compatible/self-hosted URLs
    /// remain valid, while stale `x.ai`/`grok.com` values from an old config
    /// cannot silently re-enable the hosted product.
    fn is_vendor_url(value: &str) -> bool {
    ...
    fn clean_url(value: Option<String>) -> Option<String> {
        value.filter(|value| !Self::is_vendor_url(value))
    }

    /// Remove stale hosted endpoints loaded from old config/managed layers.
    /// The field layout stays compatible with the upstream crate, but every
    /// active resolver goes through the generic local/provider defaults.
    pub fn sanitize_vendor_endpoints(&mut self) {
        self.cli_chat_proxy_base_url = Self::clean_url(self.cli_chat_proxy_base_url.take());
        self.models_base_url = Self::clean_url(self.models_base_url.take());
        self.models_list_url = Self::clean_url(self.models_list_url.take());
        self.feedback_base_url = Self::clean_url(self.feedback_base_url.take());
        self.trace_upload_url = Self::clean_url(self.trace_upload_url.take());
        self.trace_upload_bucket = Self::clean_url(self.trace_upload_bucket.take());
        self.trace_upload_endpoint_url = Self::clean_url(self.trace_upload_endpoint_url.take());
        self.managed_config_url = Self::clean_url(self.managed_config_url.take());
        self.otel_exporter_otlp_endpoint = Self::clean_url(self.otel_exporter_otlp_endpoint.take());
        self.otel_exporter_otlp_traces_endpoint =
            Self::clean_url(self.otel_exporter_otlp_traces_endpoint.take());
        self.grok_internal_otlp_traces_endpoint =
            Self::clean_url(self.grok_internal_otlp_traces_endpoint.take());
        // Deployment/management credentials are hosted-product credentials,
        // even when an old config file does not contain a vendor URL beside
        // them. Do not let those fields select a remote auth path later.
        self.alpha_test_key = None;
        self.deployment_key = None;
        self.management_api_key = None;
        self.gcs_service_account_key = None;
        if Self::is_vendor_url(&self.xai_api_base_url) {
            self.xai_api_base_url = CLI_CHAT_PROXY_BASE_URL_DEFAULT.to_owned();
        }
    }
    ...
    pub fn from_config_value(config: &toml::Value) -> Self {
        ...
        resolved.sanitize_vendor_endpoints();
        resolved
    }
    /// The OpenAI-compatible provider base URL used for local inference.
    pub fn proxy_url(&self) -> String {
        Self::clean_url(blank_as_unset(&self.cli_chat_proxy_base_url))
            .or_else(|| Self::clean_url(std::env::var("CODING_AGENT_API_BASE_URL").ok()))
            .or_else(|| Self::clean_url(std::env::var("OPENAI_BASE_URL").ok()))
            .unwrap_or_else(|| CLI_CHAT_PROXY_BASE_URL_DEFAULT.to_owned())
    }
    pub fn resolve_inference_base_url(&self) -> String {
        Self::clean_url(self.models_base_url.clone()).unwrap_or_else(|| self.proxy_url())
    }
    ...
    pub fn resolve_feedback_base_url(&self) -> String {
        Self::clean_url(blank_as_unset(&self.feedback_base_url)).unwrap_or_else(|| self.proxy_url())
    }
    /// Trace upload endpoint — an auxiliary service, so it defaults to the
    /// cli-chat-proxy, never `xai_api_base_url`.
    pub fn resolve_trace_upload_url(&self) -> String {
        Self::clean_url(blank_as_unset(&self.trace_upload_url)).unwrap_or_else(|| self.proxy_url())
    }
    /// Managed deployment configuration is intentionally disabled. A local
    /// sentinel is returned for legacy callers so they cannot fall back to a
    /// vendor endpoint.
    pub fn resolve_managed_config_url(&self) -> String {
        "http://127.0.0.1:0/disabled-managed-config".to_owned()
    }
    ...
    pub fn resolve_otlp_traces_endpoint(&self) -> String {
        if !cfg!(test) {
            return "http://127.0.0.1:0/disabled-telemetry".to_owned();
        }
    ...
    /// retained for config compatibility; local diagnostics remain independent
    ...
    /// Resolve trace bucket URL from compatibility config when explicitly
    /// requested by a test fixture; production callers receive `None`.
    ...
    pub fn resolve_models_list_url(&self) -> String {
        if let Some(url) = Self::clean_url(self.models_list_url.clone()) {
            return url;
        }
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        let mut endpoints = Self {
            cli_chat_proxy_base_url: std::env::var("CODING_AGENT_API_BASE_URL")
                .ok()
                .or_else(|| std::env::var("OPENAI_BASE_URL").ok()),
            xai_api_base_url: std::env::var("CODING_AGENT_API_BASE_URL")
                .ok()
                .or_else(|| std::env::var("OPENAI_BASE_URL").ok())
                .unwrap_or_else(|| XAI_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("CODING_AGENT_MODELS_BASE_URL"),
            models_list_url: env_string("CODING_AGENT_MODELS_LIST_URL"),
        ...
        endpoints.sanitize_vendor_endpoints();
        endpoints
    }
}
...
pub(crate) const FIRST_PARTY_CREDENTIAL_ENV_VARS: &[&str] = &[
    // Generic provider credentials are also excluded from helper processes;
    // a helper must receive only the credentials explicitly passed to it.
    crate::agent::auth_method::XAI_API_KEY_ENV_VAR,
    crate::agent::auth_method::LEGACY_XAI_API_KEY_ENV_VAR,
    // Legacy hosted names stay on the scrub list so an inherited shell
    // environment cannot leak an old account/telemetry credential into a
    // provider helper after upgrading to the clean build.
    "XAI_API_KEY",
...
pub struct Config {
    ...
    /// Legacy managed-MCP carrier retained for config compatibility; the local
    /// composition does not fetch hosted connector configs at startup.
...
impl Config {
    ...
    /// Build an account-free local `AuthManager` with the configured provider
    /// URL applied. The clean build never reads cached Grok/xAI sessions.
    pub fn create_auth_manager(&self) -> AuthManager {
        AuthManager::new_local(
    ...
    pub fn new_from_toml_cfg(raw_config: &toml::Value) -> Result<Self, String> {
        ...
        config.grok_com_config.sanitize_for_clean_build();
        config.endpoints.sanitize_vendor_endpoints();
        ...
        // Account/OIDC environment discovery is intentionally disabled. The
        // clean build accepts only generic provider credentials.
    ...
    fn apply_env_overrides(&mut self) {
        ...
        // Keep the compatibility parser for tests, but do not read or retain
        // the legacy telemetry switch in a production clean build. The
        // resolver below is fail-closed as well; this assignment prevents an
        // old environment variable from leaking into config snapshots or
        // diagnostics before the resolver runs.
        if !cfg!(test) {
            self.features.telemetry = Some(TelemetryMode::Disabled);
            return;
        }
    ...
    pub fn is_session_recap_enabled(&self) -> bool {
        // Hosted recap is an upstream product extension; local sessions do
        // not advertise or call it in the clean build.
        false
    }
    pub fn is_voice_mode_enabled(&self) -> bool {
        // Voice/STT is a removed vendor feature in the clean build. Keep the
        // resolver below for config compatibility, but never advertise it to
        // ACP clients or start a capture pipeline.
        false
    }
    ...
    pub(crate) fn resolve_telemetry_mode(&self) -> Resolved<TelemetryMode> {
        // Analytics/OTLP/Sentry are removed from the clean build. Keep the
        // resolver shape for callers that still record a decision, but never
        // let environment, managed, or remote state turn it back on.
        let _ = self;
    ...
    /// Compatibility resolver for a removed vendor marketplace gate. The clean
    /// build never auto-registers a marketplace source, regardless of env or
    /// remote settings.
    ...
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        // Image generation is a hosted vendor tool, not part of the local
        // Coding Agent surface. Keep the resolver for config compatibility,
        // but never allow env/managed/remote settings to re-enable it.
        let _ = self;
        Resolved::new(false, ConfigSource::Default)
    }
    ...
    pub(crate) fn resolve_image_gen_model_override(&self) -> Option<String> {
        // Image generation is removed from the local build. Do not let stale
        // config or managed settings select a hosted Imagine model.
        let _ = self;
        return None;
    ...
    pub(crate) fn resolve_image_edit_model_override(&self) -> Option<String> {
        // Image editing is removed from the local build; retain this resolver
        // only as a source-compatible, permanently empty API.
        let _ = self;
        return None;
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        // Server-side web/X search is an upstream hosted capability.  Keep the
        // flag and its config shape for source compatibility, but never expose
        // it from a production clean build (including when an old config or
        // environment variable asks to enable it).
        if !cfg!(test) {
            return Resolved::new(false, ConfigSource::Default);
        }
    ...
    /// Account OAuth/OIDC is disabled in the clean local build.
...
/// Sync sibling of [`is_telemetry_disabled_sync`] scoped to local error
/// reporting. Inherits from telemetry when no error-reporting-specific
/// signal is set.
...
///
/// The local telemetry table remains layered under the explicit environment
/// gate; this helper preserves requirements > env > config precedence.
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        ...
        env_bool(xai_grok_telemetry::external::config::ENV_MASTER_SWITCH),
...
/// tracing init runs).
///
/// Layering follows the local compatibility contract: **requirement > env >
/// config > default**, where the `[telemetry]` `otel_*` keys from the
/// effective config (which already includes managed-config layers distributed
/// by setup tools) sit under the env vars and requirements pins are applied on
/// top. Remote settings never control customer OTLP.
pub fn resolve_external_otel_config(
    ...
    // Keep production inert at the composition resolver as well as inside
    // the telemetry crate so startup never materializes exporter config.
    if !cfg!(test) {
        let _ = client;
        return None;
    }
...
/// re-apply. Customer OTLP remains controlled by local config and environment,
/// not remote settings.
///
...
pub(crate) fn resolve_model_list(
    ...
    if let Some(mut prefetched) = prefetched {
        ...
        for (key, entry) in prefetched.iter_mut() {
            // A stale disk cache or an old managed catalog can carry a
            // first-party hosted URL even when the top-level endpoint
            // sanitizer has already selected a local/OpenAI-compatible
            // provider. Normalize that metadata before any credential
            // resolution or sampler construction sees it.
            if EndpointsConfig::is_vendor_url(&entry.info.base_url) {
                tracing::warn!(
                    model_key = %key,
                    "ignoring removed vendor model endpoint from cached catalog"
                );
                entry.info.base_url = cfg.endpoints.resolve_inference_base_url();
            }
            if entry
                .api_base_url
                .as_deref()
                .is_some_and(EndpointsConfig::is_vendor_url)
            {
                tracing::warn!(
                    model_key = %key,
                    "ignoring removed vendor API endpoint from cached catalog"
                );
...
impl ConfigModelOverride {
    pub(crate) fn apply(
        ...
        if let Some(ref v) = self.base_url {
            if EndpointsConfig::is_vendor_url(v) {
                // A stale upstream model block must not reintroduce a hosted
                // endpoint after the top-level endpoint sanitizer ran.
                entry.info.base_url = endpoints.resolve_inference_base_url();
        ...
        if self.api_base_url.is_some() {
            entry.api_base_url = self
                .api_base_url
                .as_ref()
                .filter(|url| !EndpointsConfig::is_vendor_url(url))
                .cloned();
        }
...
/// Compatibility hook for URL-derived headers. The clean build never injects
/// provider-specific headers; model configuration owns its own header bag.
pub fn inject_url_derived_headers(
    headers: &mut IndexMap<String, String>,
    alpha_test_key: Option<&str>,
    base_url: &str,
) {
    // The upstream helper injected vendor-only token/client headers for the
    // hosted proxy. OpenAI-compatible/self-hosted providers must receive only
    // the credentials and headers explicitly configured for that model.
...
pub(crate) fn resolve_web_search_sampling_config(
    ...
    // Web/X search is a hosted vendor capability.  The clean build keeps the
    // model/config types for upstream compatibility but never constructs a
    // sampling client for this tool.
    let _ = (
        model_id,
        models,
        session_key,
...
"#;
    ...
    #[test]
    fn inject_url_derived_headers_does_not_add_vendor_headers() {
    ...
    #[test]
    fn hidden_default_web_search_resolution_is_disabled() {
        let endpoints = EndpointsConfig::default();
    ...
    #[test]
    fn resolve_model_list_sanitizes_removed_vendor_urls_in_prefetched_catalog() {
        let cfg = Config::default();
        let mut prefetched = IndexMap::new();
        prefetched.insert(
            "stale-vendor".to_string(),
            test_model_entry(
                "stale-vendor",
                "https://api.x.ai/v1",
                Some("provider-key"),
                None,
                Some("https://console.grok.build/v1"),
            ),
        );

        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = &resolved["stale-vendor"];
        assert_eq!(
            entry.info.base_url,
    ...
    fn resolve_trace_upload_explicit_config_wins_over_telemetry_off() {
        ...
        let r = cfg.resolve_trace_upload();
        assert!(!r.value, "trace upload is permanently disabled");
        assert_eq!(r.source, ConfigSource::Default);
    ...
    fn resolve_trace_upload_honors_config_when_telemetry_on() {
        ...
        cfg.telemetry.trace_upload = None;
        let r = cfg.resolve_trace_upload();
        assert!(!r.value, "trace upload remains disabled in the local build");
    }
    #[test]
    #[serial]
    ...
    fn imagine_tools_disabled_gates_image_edit() {
        ...
        let cfg = Config {
            ...
            remote_settings: Some(crate::util::config::RemoteSettings {
                imagine_tools_disabled: Some(vec!["image_edit".into()]),
                ..Default::default()
            }),
            ..Default::default()
        };
        assert!(!cfg.resolve_image_edit().value);
    }
    #[test]
    #[serial]
    fn resolve_image_gen_gates() {
        ...
        let cfg = Config {
            ...
            remote_settings: Some(crate::util::config::RemoteSettings {
                image_gen_enabled: Some(true),
                ..Default::default()
            }),
            ..Default::default()
        };
        assert!(!cfg.resolve_image_gen().value);
        assert!(!Config::default().resolve_image_gen().value);
    }
    #[test]
    #[serial]
    fn resolve_video_gen_gates() {
        ...
        let cfg = Config {
            ...
            remote_settings: Some(crate::util::config::RemoteSettings {
                video_gen_enabled: Some(true),
                imagine_tools_disabled: Some(vec!["image_to_video".into()]),
                ..Default::default()
            }),
            ..Default::default()
        };
        assert!(!cfg.resolve_video_gen().value);
    }
...
"#,
    ...
    fn external_otel_default_off_and_double_opt_in() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                None,
                ext_env(&[("XAICODE_EXTERNAL_OTEL", "1")]),
                ext_client(),
            )
            .is_none()
        );
    ...
    fn external_otel_file_table_layered_under_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                Some(&effective),
                None,
                ext_env(&[("XAICODE_EXTERNAL_OTEL", "0")]),
                ext_client(),
            )
            .is_none()
        );
    ...
    /// The local telemetry table participates in the external stream's
    /// requirement > environment > config precedence.
    #[test]
