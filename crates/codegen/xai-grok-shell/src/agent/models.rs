...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new_local(&grok_home, GrokComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    fn notify_models_updated(&self) {
        ...
        // Dynamic hosted model-push notifications are disabled. The local
        // catalog is sent once in InitializeResponse and remains config-owned.
    }
    ...
    pub(crate) fn reload_from_disk_cache(&self) {
        // Disk cache reload is local-only and remains available even though
        // the hosted model-control refresh path is disabled.
