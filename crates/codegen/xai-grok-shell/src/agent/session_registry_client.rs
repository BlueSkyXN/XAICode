...
const CLEAN_LOCAL_REGISTRY_URL: &str = "http://127.0.0.1:0/disabled-session-registry";
...
impl SessionRegistryClient {
    pub fn new(base_url: impl Into<String>, user_token: impl Into<String>) -> Self {
        ...
        Self {
            ...
            base_url: CLEAN_LOCAL_REGISTRY_URL.to_owned(),
    ...
    async fn send_authed(
        ...
        // Session replication/search/download is a hosted product service.
        // Retain the client type for local persistence compatibility, but make
        // every request fail closed before a socket or signed download URL is
        // touched.
        let _ = (&builder, op);
        return Err(anyhow::anyhow!(
            "hosted session registry is disabled in the clean local build"
        ));
