...
async fn run_headless_inner(
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a local provider API key. \
        Set CODING_AGENT_API_KEY or OPENAI_API_KEY, or use `xaicode agent stdio`.";
    ...
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!(
                    "The local provider credential is unavailable. Set CODING_AGENT_API_KEY or OPENAI_API_KEY."
                )
            }
            None => anyhow::bail!(
...
pub fn apply_otel_config(auth_manager: &AuthManager, grok_com_config: &GrokComConfig) {
    // The clean build has no external OTLP stream. Keep this public helper for
    // embedders that still call the upstream composition hook, but never open
    // the gate based on a cached session or environment key.
    let _ = (auth_manager, grok_com_config);
    suppress_otel();
}
