...
pub(crate) async fn single_check(
    ...
    // Subscription/entitlement polling is a hosted account feature. Keep the
    // parser and test seam, but never issue `/user?include=subscription` in a
    // production clean build.
    if !cfg!(test) {
        let _ = (auth_manager, proxy_base_url, alpha_test_key, user_id);
        return None;
    }
