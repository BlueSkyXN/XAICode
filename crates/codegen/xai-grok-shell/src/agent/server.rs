            ...
            let _agent_thread = thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    local_set.block_on(&rt, async move {
                        run_persistent_agent(agent_config, conn_rx, None).await
                    });
...
async fn run_persistent_agent(
    ...
    // The persistent server uses the same account-free local manager as stdio;
    // it never refreshes hosted credentials, managed policy, or OTLP config.
    let agent = Rc::new(
