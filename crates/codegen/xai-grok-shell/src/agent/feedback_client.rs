...
impl FeedbackClient {
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        // Feedback/analytics transport is disabled in the clean build. Keep
        // the old headers only for compatibility tests; production callers
        // must not even construct a vendor-marked request.
        if !cfg!(test) {
            return builder;
        }
        let builder = builder
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        if !cfg!(test) {
            let _ = (&self, request);
            anyhow::bail!("hosted feedback and analytics are disabled in the clean local build");
        }
