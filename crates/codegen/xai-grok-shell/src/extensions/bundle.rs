...
pub(crate) const BUNDLE_DISABLED_ERROR: &str =
    "Hosted bundled agents are disabled in the clean local build.";
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !cfg!(test) {
        return Err(acp::Error::method_not_found().data(BUNDLE_DISABLED_ERROR));
    }
...
async fn sync_bundle(agent: &MvpAgent, req: BundleSyncRequest) -> anyhow::Result<BundleSyncResult> {
    if !cfg!(test) {
        anyhow::bail!(BUNDLE_DISABLED_ERROR);
    }
