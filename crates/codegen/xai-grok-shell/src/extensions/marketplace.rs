...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !cfg!(test) {
        ...
        return Err(acp::Error::method_not_found()
            .data("The vendor marketplace is disabled in the clean local build."));
    }
...
async fn handle_add_source(url: &str) -> xai_hooks_plugins_types::ActionOutcome {
    ...
    if let MarketplaceAddInput::GitUrl(git_url) = &input
        && xai_grok_plugin_marketplace::is_blocked_vendor_source_url(git_url)
    {
        return ActionOutcome {
            status: OutcomeStatus::ValidationError,
            message: "Vendor-owned marketplace sources are disabled in the clean local build. Add a local or provider-neutral source instead.".into(),
            requires_reload: false,
            requires_restart: false,
        };
    }
                ...
                return ActionOutcome {
                    ...
                    message: format!(
                        "{e}. Not a reachable git repository — to add it anyway (e.g. a \
                         VPN-gated host), run: xaicode plugin marketplace add {url} --force"
                    ),
                    requires_reload: false,
                    requires_restart: false,
                };
    ...
    let write = {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            add_marketplace_source(&config_path, &name, &input, false)
        })
        .await
    };
    match write {
...
fn remove_source_locked(source_url_or_path: &str) -> xai_hooks_plugins_types::ActionOutcome {
    ...
    // Remove only the requested source. The clean build has no sticky
    // vendor-source flag and never re-adds a source during startup.
...
pub fn ensure_official_marketplace_source(grok_home: &std::path::Path) {
    // The upstream helper is retained only for old unit fixtures and source
    // compatibility. Production builds never auto-register a vendor source;
    // custom marketplace entries must be added explicitly by the user.
    if !cfg!(test) {
        let _ = grok_home;
        tracing::debug!("vendor marketplace auto-registration is disabled");
        return;
    }
    let config_path = grok_home.join("config.toml");
