...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !cfg!(test) {
        ...
        return Err(acp::Error::method_not_found()
            .data("Session sharing is disabled in the clean local build."));
    }
...
fn require_xai_auth_for_share(
    ...
    super::auth_gate::require_xai_auth(
        ...
        "Share session is disabled in the clean local build.",
    )
}
...
mod tests {
    ...
    fn share_rejects_non_xai_auth_with_actionable_grok_login_message() {
        ...
        assert_eq!(data, "Share session is disabled in the clean local build.");
    }
}
