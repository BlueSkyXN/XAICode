...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Shared delete: local disk + FTS eviction.
...
async fn handle_update_mcp_servers(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let admitted =
        crate::session::mcp_sources::admit_client_mcp_servers(params.mcp_servers, &cwd, &compat);
...
async fn handle_reload_all_mcp_servers(agent: &MvpAgent) -> ExtResult {
    ...
    for session_id in &session_ids {
        ...
        // `merge_mcp_servers` already re-reads every disk source
        ...
        if crate::session::mcp_sources::merge_and_send_mcp_update(
            &handle.cmd_tx,
            &cwd,
...
async fn handle_session_fork(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let response = fork_session(request, Some(agent.auth_manager.clone()))
        .await
        .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
