...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !cfg!(test) {
        ...
        return Err(acp::Error::method_not_found()
            .data("Usage and billing are disabled in the clean local build."));
    }
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Billing data is disabled in the clean local build.",
    )?;
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Auto top-up data is disabled in the clean local build.",
    )?;
