...
/// Session rate-limit copy retained for protocol compatibility. The clean
/// build does not advertise an OAuth/session method.
pub const RATE_LIMITED_USER_MESSAGE_OAUTH: &str =
    "You\u{2019}ve hit the provider rate limit. Try again later.";

/// API-key rate-limit copy for a generic provider.
pub const RATE_LIMITED_USER_MESSAGE_API_KEY: &str =
    "You\u{2019}ve hit the provider API rate limit. Try again later.";
...
/// User-facing free-usage exhaustion copy. Billing/upsell links are removed.
pub const FREE_USAGE_USER_MESSAGE: &str =
    "You\u{2019}ve reached the provider usage limit for now. Try again later.";
...
pub fn is_free_usage_exhausted_error(detail: &str) -> bool {
    // The subscription/paywall marker is an upstream account protocol. A
    // clean build treats the same provider response as an ordinary rate
    // limit, while tests retain the original parser seam.
    if !cfg!(test) {
        let _ = detail;
        return false;
    }
...
///
/// Free-usage code first (provider compatibility). Else the body
...
/// Detect subscription upsell text from a provider response so the clean build
/// can replace it with neutral rate-limit wording instead of linking to a
/// vendor account page.
fn pushes_consumer_subscription_upsell(detail: &str) -> bool {
    let d = detail.to_ascii_lowercase();
    d.contains("upgrade to a subscription") || d.contains("upgrade your plan")
}
    ...
    match err {
        ...
        SamplingError::Api {
            ...
            StatusCode::FORBIDDEN => {
                let message = if message.contains("requires a subscription")
                    && crate::agent::auth_method::has_xai_api_key_env()
                {
                    "The provider requires a subscription for this operation; \
                     account login is disabled in the clean local build."
                        .to_string()
                } else {
                    message
                };
