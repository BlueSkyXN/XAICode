...
pub enum WarningCategory {
    ...
    /// The session runs over SSH without `xaicode wrap` on the local end, so
    ...
    if ctx.brand == TerminalName::AppleTerminal && ctx.is_ssh {
        ...
        warning.note = Some(
            "Grok also saves each copy to the backup file shown in the copy message. To copy \
             directly, run `xaicode wrap ssh <host>` on your local computer or use a terminal that \
...
/// Pure SSH `xaicode wrap` recommendation — suggests launching the session
/// through `xaicode wrap ssh <host>` on the user's local machine, which gives a
...
mod tests {
    ...
    // -- ssh_wrap_hint: `xaicode wrap ssh` recommendation ------------------
    ...
    fn ssh_wrap_hint_fires_over_plain_ssh() {
        ...
        assert_eq!(w.fix.as_deref(), Some("xaicode wrap ssh <host>"));
        assert!(
    ...
    fn ssh_wrap_hint_suppressed_when_sink_active() {
        ...
        // `xaicode wrap` — adoption silences the hint by itself.
