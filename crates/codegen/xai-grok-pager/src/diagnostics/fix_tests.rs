...
fn bash_zsh_and_fish_plans_use_exact_paths_and_aliases() {
    ...
    for (shell, relative, alias) in [
        ("/bin/bash", ".bashrc", "alias ssh='xaicode wrap ssh'"),
        ("/bin/zsh", ".zshrc", "alias ssh='xaicode wrap ssh'"),
        (
            "/usr/local/bin/fish",
...
fn shell_aliases_expand_to_exact_argv_and_bypass_is_explicit() {
    ...
    if let Some(bash) = find_on_path("bash") {
        ...
        std::fs::write(&rc, "alias ssh='xaicode wrap ssh'\n").unwrap();
        let command = format!(
    ...
    let mut shell = std::process::Command::new(bash);
    shell
        .args(["-ic", "alias ssh='xaicode wrap ssh'; command ssh host"])
        .env("CAPTURE", &capture)
        .env(
            "PATH",
            format!(
    ...
    if let Some(fish) = find_on_path("fish") {
        ...
        std::fs::write(&rc, "alias ssh 'fish-xaicode wrap ssh'\n").unwrap();
        let command = format!(
