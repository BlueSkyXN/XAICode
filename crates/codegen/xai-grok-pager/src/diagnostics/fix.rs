...
pub const SSH_WRAP_ONE_OFF: &str = "xaicode wrap ssh <host>";

const MANAGED_NAMESPACE: &str = "grok doctor";
const SSH_WRAP_ALIAS_POSIX: &str = "alias ssh='xaicode wrap ssh'";
const SSH_WRAP_ALIAS_FISH: &str = "alias ssh 'xaicode wrap ssh'";
...
pub(crate) fn format_fix_preview(plan: &FixPlan) -> String {
    ...
    match &plan.payload {
        FixPayload::SshWrap(_) => {
            output.push_str(
                "\nWhat this changes:\n  In new interactive shells, `ssh ...` runs as `xaicode wrap ssh ...`.\n",
            );
            let _ = writeln!(
                output,
...
fn plan_ssh_wrap(
    ...
    Ok(FixPlan {
        ...
        caveats: vec![
            ...
            "`xaicode wrap` starts the SSH process directly, so the alias does not loop.",
