//! `xaicode plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
/// Typed entry for `xaicode plugin list --json`. The `status` field acts as a
...
/// Typed entry for `xaicode plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `xaicode plugin list`).
        name: String,
...
fn trust_prompt(subject: &str, source_arg: &str) -> String {
    format!(
        "Installing {subject} requires confirmation.\n\
         ...
         \n\
         To proceed, re-run with --trust:\n  xaicode plugin install {source_arg} --trust"
    )
}
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `xaicode plugin install --help` to get started.");
    } else {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `xaicode plugin update {}` to update it.",
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               xaicode plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `xaicode plugin tag`."
            )
        })?,
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `xaicode plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
...
mod tests {
    ...
    fn trust_prompt_marketplace_has_no_error_framing() {
        ...
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  xaicode plugin install sentry@xai-org/plugin-marketplace --trust"
        ));
    ...
    fn trust_prompt_git_and_local_subjects() {
        ...
        assert!(
            git.ends_with("  xaicode plugin install u/r --trust"),
            "{git}"
        );
