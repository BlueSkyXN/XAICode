...
fn mixed_report() -> DiagnosticReport {
    ...
    report.findings = vec![
        ...
        DiagnosticFinding {
            ...
            message: "Use local SSH wrapping".to_owned(),
            remediation: Some(ManualRemediation {
                fix: "xaicode wrap ssh <host>".to_owned(),
                config_path: None,
            }),
            automatic_remediation: Some(crate::diagnostics::ssh_wrap_automatic_remediation()),
            note: None,
...
fn fix_preview_contains_exact_change_and_caveats() {
    ...
    assert!(preview.contains(
        "# >>> grok doctor >>>\n# >>> terminal.ssh-wrap >>>\nalias ssh='xaicode wrap ssh'"
    ));
    assert!(preview.contains("To use once without changing config: `xaicode wrap ssh <host>`"));
        ...
        serde_json::json!({
            ...
            "findings": [
                ...
                },
                {
                    ...
                    "remediation": {"fix": "xaicode wrap ssh <host>", "configPath": null},
                    "automaticRemediation": {
...
fn new_named_findings_extend_json_without_schema_changes() {
    ...
    report.facts.clipboard.fix = Some("xaicode wrap <ssh command> or /minimal".to_owned());
    report.findings.push(DiagnosticFinding {
    ...
    assert_eq!(
        ...
        "xaicode wrap <ssh command> or /minimal"
    );
