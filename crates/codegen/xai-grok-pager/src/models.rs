...
pub async fn list_available_models(agent_config: &AgentConfig) -> Result<()> {
    match AuthStatus::resolve(agent_config) {
        AuthStatus::ApiKey => println!("You are using the configured provider API key."),
        // Account/deployment variants are retained in the upstream enum for
        // compatibility, but the clean runtime never constructs them.
        AuthStatus::LoggedIn(host) => println!("Provider session: {}.", host),
        AuthStatus::ModelCredentials(model) => {
            println!("Model '{model}' is using its own API key.");
        }
        AuthStatus::DeploymentKey => println!("A deployment credential is configured."),
