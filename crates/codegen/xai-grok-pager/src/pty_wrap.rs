//! Local PTY wrapper: the engine behind `xaicode wrap` (see [`crate::wrap_cmd`]).
//!
...
///
/// This is the engine behind `xaicode wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // detached — `xaicode wrap` is short-lived and exits with the child.
