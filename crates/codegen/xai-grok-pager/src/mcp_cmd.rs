//! `xaicode mcp` — manage MCP server configurations from the command line.
...
Examples:
  ...
  xaicode mcp add xcode -- xcrun mcpbridge
  ...
  xaicode mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
...
pub struct AddArgs {
    ...
    /// flags such as `-y` are passed to the server instead of xaicode.
    #[arg(value_name = "ARGS")]
    args: Vec<String>,
...
fn run_list(json: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No MCP servers configured. Run `xaicode mcp add --help` to get started.");
    } else {
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: xaicode mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: xaicode mcp add --transport {label} <name> <url>"
                );
            };
...
/// TOML / disabled list / compat JSON / plugin names.
...
async fn run_set_enabled(name: &str, enabled: bool) -> Result<()> {
    ...
    if !mcp_server_is_known(name, &cwd) {
        ...
        if !available.is_empty() {
            ...
            eprintln!("No MCP servers configured. Run `xaicode mcp add --help` to get started.");
        }
        std::process::exit(1);
    }
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let (scope, path) = match select_remove_site(user_defined, find_project_site(), requested_scope)
    {
        ...
        Err(RemoveError::Ambiguous { project_path }) => {
            ...
            eprintln!(
                "Specify which one to remove, e.g.: xaicode mcp remove {name} --scope project"
            );
            std::process::exit(1);
        }
    };
