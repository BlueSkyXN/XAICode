...
fn auth_required_message(interactive: bool) -> String {
    let _ = interactive;
    "No provider API key is configured. Set CODING_AGENT_API_KEY or OPENAI_API_KEY, \
     or add api_key/env_key to the selected model in config.toml."
        .to_string()
}
...
/// `--worktree` is ignored here: headless never creates a worktree, so a local
...
pub async fn run_single_turn(
    ...
    // hub_url belonged to the removed hosted workspace command.
