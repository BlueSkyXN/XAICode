...
//!
//! Shows a compact usage status item when a caller supplies local usage data.
//! The clean build does not fetch account, subscription, or billing data.
...
/// Compatibility shape for usage data supplied by an embedding host.
///
/// The local clean build does not populate this account/billing-shaped value;
/// the session `/usage` command uses its in-process token ledger instead.
#[derive(Debug, Clone)]
