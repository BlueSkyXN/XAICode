...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: local provider state and the local agent version.
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: local channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: local agent version (left-aligned).
    HeroInline,
}
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = params.login_label.unwrap_or("provider API key");
            let configure_text = format!("Configure {}", label);
            let menu = [("l", configure_text.as_str()), ("q", "Quit")];
        ...
        AuthState::Authenticating { auth_url, mode, .. } => {
            ...
            let (url_rect, fallback_rect) = render_welcome_authenticating(
                ...
                &theme,
                llc,
                // A stale upstream auth state may still carry a browser URL.
                // The clean runtime never renders or follows it; keep the
                // value only for compatibility tests of the renderer.
                auth_url.as_deref().filter(|_| cfg!(test)),
                *mode,
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "This local XAICode configuration is not available.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that the agent
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "The XAICode may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_browser_status_arm(
    ...
    let (header, waiting_text, user_code) = match kind {
        BrowserStatusKind::Command => (AUTH_HEADER, "Waiting for provider authentication...", None),
...
fn render_welcome_done(
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        let gate_text = p
            ...
            .unwrap_or("Local provider access is not configured");
        let msg = Line::from(Span::styled(
            gate_text,
            Style::default().fg(theme.gray_bright),
        ))
        ...
        if centered.height > 2 {
            ...
            // Never render or make clickable an account/subscription URL from
            // a stale hosted gate in the clean local build.
            let gate_link = "";
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("XAICode"), "full badge: {full:?}");
        assert!(inline.contains("XAICode"), "inline badge: {inline:?}");
