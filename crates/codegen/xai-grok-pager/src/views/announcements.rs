...
/// Hosted vendor destinations are not part of the clean local agent. Keep
/// announcement overrides useful for local documentation/support links while
/// preventing a stale or user-supplied promo from reopening the old account,
/// billing, or marketing surfaces.
fn is_blocked_vendor_url(value: &str) -> bool {
...
fn usable_cta(a: &xai_grok_announcements::RemoteAnnouncement) -> Option<(&str, &str)> {
    ...
    let url = cta
        ...
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .filter(|u| !is_blocked_vendor_url(u))
        .filter(|u| {
...
mod tests {
    ...
    fn usable_cta_rejects_unsafe_schemes() {
        ...
        for good in ["https://example.com/promo", "http://localhost:3000/help"] {
        ...
        for blocked in [
            ...
            let a = promo("p", "msg", Some(("Go", blocked)));
            assert!(
                usable_cta(&a).is_none(),
                "hosted vendor CTA must be rejected: {blocked}"
            );
        }
