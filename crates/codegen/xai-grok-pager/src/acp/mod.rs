...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    // Hosted workspace-start arguments are no longer part of this build.
...
pub async fn connect_via_leader(
    ...
    let mut agent_config = AgentConfig::new_from_toml_cfg(raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    // Hosted leader settings are disabled in the clean build.
    agent_config.remote_settings = None;
...
/// Build `client_capabilities.meta` for the local ACP client.
///
/// The upstream client advertised a large `x.ai/*` capability contract. Those
/// keys are product-specific and have no meaning for a provider-neutral agent,
/// so the clean build intentionally sends an empty metadata object. Standard
/// ACP capabilities continue to be advertised by the surrounding request.
...
async fn initialize(
    ...
    // Check if this is the in-process coding agent.
    let is_grok_shell = resp
        .meta
        .as_ref()
        .and_then(|m| m.get("codingAgent").or_else(|| m.get("grokShell")))
...
async fn eager_auth_or_login_fallback(
    ...
    if auth_methods.is_empty() {
        // No interactive login exists in the clean build. Leave the session
        // usable so a request can report the missing local provider key.
