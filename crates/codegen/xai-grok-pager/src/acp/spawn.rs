...
pub async fn spawn_grok_shell(
    ...
    // The original path loaded auth.json, refreshed account tokens, fetched
    // managed policy and opened an OTLP gate here. The clean agent owns no
    // account session; its AuthManager is an in-memory local-key container.
