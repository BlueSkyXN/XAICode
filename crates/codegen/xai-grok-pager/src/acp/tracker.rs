...
/// rather than a media file (for example, a provider-disabled response).
...
[CI] IN_PROGRESS=5 SUCCESS=38\n\
    ...
    /// A disabled media call can return a plain-text explanation on a
    /// `Completed` status. The media renderer has no file to open, so it must surface the text in the
    /// card body (not a bare title) and must NOT mark the card as an error.
    #[test]
    fn disabled_media_shows_explanation_not_error() {
        let explanation = "Image generation is disabled in the clean local build.";
        let output = ToolOutput::Text(xai_grok_tools::types::output::TextOutput::from(explanation));
        let tc = acp::ToolCall::new(
        ...
        .content(vec![acp::ToolCallContent::Content(acp::Content::new(
            acp::ContentBlock::Text(acp::TextContent::new(explanation)),
        ))])
        ...
        assert!(
            block
                ...
                .as_deref()
                .unwrap_or_default()
                .contains("disabled in the clean local build"),
            "provider explanation must be shown in the card body, got: {:?}",
            block.output
        );
    }
}
