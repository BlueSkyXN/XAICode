...
/// Top-level commands for the local xaicode binary.
#[derive(Debug, Clone, Subcommand)]
pub enum Command {
    /// Run the agent without the interactive UI.
    Agent(Box<AgentArgs>),
    ...
    /// Check terminal, clipboard, color, and input support without starting the agent.
    Doctor(crate::doctor_cmd::DoctorArgs),
    /// Manage running leader processes (disabled in the clean build).
    #[command(hide = true)]
...
Examples:
  xaicode wrap docker exec -it my-container bash
  xaicode wrap kubectl exec -it my-pod -- bash

See ~/.grok/README.md for more information. The directory name is retained
for compatibility with existing sessions and configuration files.
")]
    Wrap(WrapArgs),
    ...
    /// `~/.grok/config.toml` or when the `CODING_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
/// Targets a running leader process by PID (used by the hidden local leader
/// maintenance command).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `xaicode leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "xaicode: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("xaicode: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over stdio (no hosted relay)
    Headless(HeadlessArgs),
    /// Run the agent as a WebSocket server
    Serve(ServeArgs),
    /// Run as the shared leader process for other clients (disabled).
    #[command(hide = true)]
    Leader(LeaderArgs),
}
...
pub struct HeadlessArgs {
    /// Legacy relay field retained only for source compatibility; not parsed.
    #[arg(skip)]
...
pub struct LeaderArgs {
    ...
    /// Retained for CLI compatibility; hosted relay support is disabled.
    #[arg(long)]
    pub relay_on_demand: bool,
    /// Retained for CLI compatibility; automatic updates are disabled.
    #[arg(long)]
    pub no_auto_update: bool,
    /// Local endpoint overrides (passed from a follower process).
    #[command(flatten)]
    pub headless: HeadlessArgs,
}
...
pub struct PagerArgs {
    ...
    /// Default for all `xaicode -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    /// Session-scoped only — does not write config. To default plain `xaicode` to
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `xaicode "fix the bug"` or `xaicode --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    /// whitespace. This is the `xaicode "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn subcommand_takes_precedence_over_positional_prompt() {
        let args = PagerArgs::try_parse_from(["grok", "models"]).expect("subcommand parses");
