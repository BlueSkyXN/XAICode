...
pub(crate) async fn run(
    ...
    // memory growth.
    // Product log sampling is intentionally ignored. Tracing remains local
    // and is never exported to a vendor or hosted control plane.
    ...
    // Account/OIDC login is removed from the clean build.
    app.auth_use_oauth = false;
    app.show_resolved_model = remote_settings
        .as_ref()
        .and_then(|s| s.show_resolved_model)
        .unwrap_or(true);
    // Session sharing/relay is disabled in the local product, regardless of
    // stale remote-settings payloads.
    app.sharing_enabled = false;
    // Product privacy/banner/plugin upsell controls are not part of the clean
    // local agent. Keep all related UI suppressed even when old env/config
    // keys are present.
    ...
    // There is no browser/OIDC login screen in the clean build. Missing keys
    // are reported by the provider request instead of opening account UI.
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            // Compatibility-only force-login path: find an advertised local method.
            let grok_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "api_key");
            if let Some(method) = grok_com {
                ...
                // No local API-key method available, use the first method as fallback
    ...
    // part of the local build. Keep ordinary user config for UI preferences,
    // but never read the product-control layers or announcement state.
    app.hidden_announcement_ids = Default::default();
    let requirements: Option<toml::Value> = None;
    let user_config = xai_grok_shell::config::load_from_disk().ok();
    let managed_config: Option<toml::Value> = None;
...

    {
        ...
        // Product announcements and promotional tips are intentionally empty.
    ...
    // `xaicode dashboard` startup: open the dashboard view immediately.
    // The CLI subcommand wrote a `CODING_AGENT_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    let open_dashboard_at_startup = std::env::var("CODING_AGENT_OPEN_DASHBOARD_AT_STARTUP")
        .as_deref()
        == Ok("1")
        || (cfg!(test) && std::env::var("GROK_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1"));
    if open_dashboard_at_startup {
        // SAFETY: we are pre-multithreaded init for this app loop.
        unsafe { std::env::remove_var("CODING_AGENT_OPEN_DASHBOARD_AT_STARTUP") };
        #[cfg(test)]
        {
