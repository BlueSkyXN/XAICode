...
/// Hosted subscription tiers do not exist in the clean local build. The
/// constant remains so the upstream command-registry plumbing compiles, but no
/// local command is hidden or unlocked from account metadata.
...
pub struct AppView {
    ...
    /// Whether the pager is connected via a leader (leader mode). The local
    /// dashboard/session picker is available in either mode; this flag only
    /// selects whether its roster comes from a live leader or the local
    /// on-disk session list. Set in `event_loop::run` from
    /// `connection.leader_status_rx.is_some()`.
    ...
    /// One-shot gate for the SSH `xaicode wrap` tip: set after the first
...
impl AppView {
    ...
    pub fn gate_from_settings(
        ...
        // Remote access gates are a hosted account/billing surface. The
        // clean local build never turns stale settings (or a test fixture
        // accidentally handed to a production caller) into a paywall.
        if !cfg!(test) {
            let _ = rs;
            return None;
        }
    ...
    pub fn apply_auth_meta(&mut self, meta: &xai_grok_shell::auth::AuthMeta) {
        ...
        // Account/team, paywall, data-retention and subscription metadata are
        // ignored. A stale upstream ACP response must not turn the local TUI
        // into an account surface or apply hosted tier restrictions.
        let _ = meta;
        self.team_id = None;
        self.team_name = None;
        self.is_zdr = false;
    ...
    pub(crate) fn sync_billing_surface_to_agents(&mut self) {
        ...
        // `/usage` is local token/context information, so custom external
        // providers keep the command even though hosted billing stays off.
        let usage_cmd = true;
...
impl AppView {
    ...
    /// One-shot SSH `xaicode wrap` tip trigger, run at the top of every `draw`
...
pub(crate) mod tests {
    ...
    #[test]
    fn apply_auth_meta_keeps_billing_surface_disabled_for_personal_users() {
        let mut app = test_app();
        app.usage_visible = false;
    ...
    #[test]
    fn apply_auth_meta_clears_api_key_flag_without_restoring_billing() {
        let mut app = test_app();
