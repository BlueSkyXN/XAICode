...
pub(crate) fn execute(
    ...
    match effect {
        ...
        Effect::SwitchAccount { request_seq, method_id: _, use_oauth: _ } => {
            let abort_handle = tasks.spawn(async move {
                TaskResult::AuthFailed {
                    request_seq,
                    error: "Account switching is disabled in the clean local build.".to_owned(),
                }
            });
        ...
        Effect::Authenticate {
            ...
            if !cfg!(test) {
                let abort_handle = tasks.spawn(async move {
                    let _ = (method_id, use_oauth, force_interactive);
                    TaskResult::AuthFailed {
                        request_seq,
                        error: "Interactive account authentication is disabled in the clean local build. Configure CODING_AGENT_API_KEY or OPENAI_API_KEY.".to_owned(),
                    }
                });
                meta.auth_abort_handle = Some((request_seq, abort_handle));
            } else {
                let tx = acp_tx.clone();
                let abort_handle = tasks.spawn(async move {
                    send_authenticate(&tx, request_seq, method_id, use_oauth, force_interactive)
                        .await
                });
                meta.auth_abort_handle = Some((request_seq, abort_handle));
            }
        ...
        Effect::SubmitAuthCode { request_seq, code } => {
            ...
            tasks.spawn(async move {
                TaskResult::AuthFailed {
                    request_seq,
                    error: "Interactive account authentication is disabled in the clean local build."
                        .to_owned(),
                }
            });
        }
        ...
        Effect::SetCodingDataSharing {
            ...
            // The upstream action posted a hosted retention preference. The
            // clean build never sends coding data or consent state; report a
            // local failure so any stale optimistic UI rolls back cleanly.
            tasks.spawn(async move {
                TaskResult::CodingDataSharingFailed {
                    agent_id,
                    error: "Coding-data sharing is unavailable in the clean local build."
                        .to_owned(),
                    rollback_to_opted_in,
                    seq,
                }
        ...
        Effect::SendRecap { session_id, auto } => {
            tasks.spawn(async move {
                TaskResult::RecapRequested {
                    session_id,
                    auto,
                    error: Some("Session recap is disabled in the clean local build.".to_owned()),
                }
            });
        }
...
///
/// The clean build reports only the local provider credential state. It never
/// points users to an account, billing, or hosted login surface.
fn format_auth_lines(is_api_key_auth: bool, api_key_env_set: bool) -> String {
    if is_api_key_auth {
        let method = if api_key_env_set {
            "  Auth method: API key (CODING_AGENT_API_KEY/OPENAI_API_KEY)\n"
        } else {
            "  Auth method: API key\n"
        };
        return method.to_owned();
    }
    String::from("  Auth method: local provider (no account session)\n")
}
