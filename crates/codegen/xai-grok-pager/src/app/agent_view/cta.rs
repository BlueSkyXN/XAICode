...
impl AgentView {
    ...
    pub(in crate::app) fn connect_matched_plugin(&mut self) {
        // The unsolicited vendor-marketplace CTA is removed from the clean
        // build. Explicit marketplace installs continue through `/plugin` and
        // the extensions modal, so a stale phase is simply dismissed here.
