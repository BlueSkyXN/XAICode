...
impl AppView {
    ...
    pub fn subscription_watch_wanted(&self) -> bool {
        // Account subscription state is an upstream-only concept. Keep the
        // original state machine for unit tests and wire compatibility, but
        // make the production build permanently local and gate-free.
        if !cfg!(test) {
            return false;
        }
        self.usage_visible
    ...
    pub fn lift_gate(&mut self) -> Vec<Effect> {
        if !cfg!(test) {
            // Clear stale serialized state, but never schedule an upstream
            // subscription check in the clean build.
            self.gate = None;
