...
///
/// Returns `Ok(false)` after the local TUI exits. The boolean is retained for
/// source compatibility with the original composition root; update/relaunch
/// handling is intentionally absent in the clean build.
pub async fn run(args: PagerArgs) -> anyhow::Result<bool> {
    ...
    // No account refresh, hosted settings, or product model prefetch occurs at
    // startup. The TUI receives only local configuration.
    ...
    let disabled_by_confinement: Option<&str> = None;
    tracing::info!(
        use_leader,
        ?policy_disable_reason,
    ...
    match result {
        Ok(run_result) => {
            ...
            // updater or staged binary to relaunch in the clean build.
...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  xaicode --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  xaicode --resume {}", info.session_id);
    }
}
