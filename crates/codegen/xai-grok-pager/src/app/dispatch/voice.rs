...
/// Compatibility shim for the removed hosted voice upsell.
///
/// Voice is not part of the local Coding Agent product. Keep this function so
/// old action plumbing still links, but never render an account or upgrade
/// surface.
...
///
/// The upstream implementation was gated by remote settings and subscription
/// state. The clean build keeps the action for compatibility but always returns
/// before microphone capture, provider authentication, or UI/upsell work.
...
pub(super) fn dispatch_enable_voice_mode(app: &mut AppView, from_hold: bool) -> Vec<Effect> {
    // The upstream voice/upgrade path is intentionally removed. Keep the
    // action for wire compatibility, but fail closed before microphone,
    // account, or hosted billing code can run even if stale settings arrive.
    let _ = (app, from_hold);
    return vec![];
    ...
    #[allow(unreachable_code)]
    {
        ...
        // Legacy tier gate retained only in unreachable compatibility code.
