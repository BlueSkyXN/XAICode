...
pub(super) fn plugin_cta_candidates(
    _response: xai_hooks_plugins_types::MarketplaceListResponse,
) -> (Vec<xai_hooks_plugins_types::MarketplacePluginEntry>, bool) {
    // The upstream implementation offered an unsolicited install CTA for a
    // vendor-owned marketplace. Marketplace browsing and explicit installs
    // remain available through the normal commands/modal; the clean build
    // does not privilege or auto-install any source.
    (Vec::new(), false)
}
