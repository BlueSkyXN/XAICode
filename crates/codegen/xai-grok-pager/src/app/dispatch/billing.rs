...
pub(crate) fn is_credit_limit_error(http_status: Option<u16>, message: &str) -> bool {
    // Credit balances, spend caps, and upgrade prompts belong to the removed
    // hosted account product. Keep the classifier for compatibility tests,
    // but never route a production provider error into that UI state.
    if !cfg!(test) {
        let _ = (http_status, message);
        return false;
    }
...
pub(super) fn open_credit_limit_upsell(
    ...
    let _ = (agent, mode, max_tier);
    // Hosted billing/upgrade cards are removed. Provider rate-limit errors are
    // rendered as ordinary system errors by the prompt dispatcher.
    return;
...
fn open_supergrok_upsell(
    ...
    // No account/billing UI exists in the clean local build.
    return false;
...
pub(super) fn handle_billing_fetched(
    ...
    if !app.usage_visible {
        // Ignore stale billing responses after the local-only policy has
        // hidden the account surface. This also prevents a late response from
        // re-enabling polling or populating credit state.
...
pub(super) fn dispatch_open_supergrok_url(app: &mut AppView) -> Vec<Effect> {
    let _ = app;
    // Kept as an action-compatible stub so the upstream dispatch enum remains
    // source-compatible. The clean build never opens a hosted billing or
    // upgrade page.
    vec![]
}
