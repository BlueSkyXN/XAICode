...
/// Whether a completed clipboard probe should fall through to the `xaicode wrap`
/// host-image request. A clean `FullMiss` always qualifies; a remote read
/// *error* (`AttachmentRead`) also qualifies because inside `xaicode wrap` the
...
/// `osc52_sink_active()`, so this is inert outside `xaicode wrap`.
