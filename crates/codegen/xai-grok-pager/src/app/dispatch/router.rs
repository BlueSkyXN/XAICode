...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = match action {
        ...
        // Account login/logout and hosted account switching are intentionally
        // removed from the local build. Keep the action variants for wire
        // compatibility, but make every stale keyboard/ACP action fail
        // closed instead of reaching the legacy OAuth/device-code flow.
        Action::Logout
        ...
        Action::OpenLink(target) => {
            ...
            match crate::render::osc8::resolve_link_open_target(&target) {
                ...
                Some(LinkTarget::Url(url)) => {
                    // Route OSC-8/user-visible links through the shared
                    // scheme/host gate. This keeps stale vendor URLs from
                    // bypassing the clean-build opener policy.
                    open_url_or_show(app, &url);
                }
                None => {}
            }
