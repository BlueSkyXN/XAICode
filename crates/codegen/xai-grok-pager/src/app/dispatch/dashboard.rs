...
pub(super) fn dispatch_open_dashboard(app: &mut AppView) -> Vec<Effect> {
    ...
    // Gate behind provider setup. Until the local provider is ready, the
    // backend rejects new sessions; activating the dashboard view would
    // visually dismiss the setup UI. Toast and stay put.
    if !matches!(app.auth_state, crate::app::app_view::AuthState::Done) {
        app.show_toast("Configure a provider API key to open the dashboard");
        return vec![];
    }
...
pub(super) fn dispatch_dashboard_dispatch_slash(app: &mut AppView, text: String) -> Vec<Effect> {
    ...
    let result = {
        ...
        // Keep the stale restriction branch source-compatible, but do not
        // expose the upstream account/upgrade product surface. The clean
        // registry has no restricted hosted commands; this is only a final
        // fail-closed guard for stale metadata.
        if reg.is_restricted(invocation.token) {
            let token = invocation.token.to_string();
            if let Some(d) = app.dashboard.as_mut() {
                d.dispatch.set_text("");
                d.set_error_toast(&format!("/{token} is disabled in the local build"));
            }
            return vec![];
        }
