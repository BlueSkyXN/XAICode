...
//
// Concrete codes match the optional provider-neutral STT language catalog.
...
pub fn default_settings() -> Vec<SettingMeta> {
    ...
    vec![
        ...
        SettingMeta {
            ...
            description: "The clean local build never uploads coding data, prompts, traces, or \
                          usage metrics. This legacy preference is retained only for config \
                          compatibility and is not shown in the shipped settings registry.",
            keywords: &[
                "privacy",
                "data",
                "sharing",
        ...
        SettingMeta {
            ...
            owner: SettingOwner::Shell,
            label: "SSH wrap",
            description: "Show a `/doctor` tip when an SSH session is not using `xaicode wrap`.",
            keywords: &[
                "ssh",
                "wrap",
                "remote",
