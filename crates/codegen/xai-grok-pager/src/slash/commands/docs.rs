...
//! `/docs web` is retained as a compatibility command but has no hosted URL
//! in the local build.
...
/// Hosted Build docs are intentionally not bundled into the local build.
    ...
    fn suggest_args(&self, _ctx: &AppCtx, _args_query: &str) -> Option<Vec<ArgItem>> {
        let mut items = vec![
            ...
            ArgItem {
                ...
                description: "Hosted docs are disabled in the local build".into(),
            },
        ];
