...
pub struct CommandRegistry {
    ...
    /// Commands denied by a local policy (for example, a tool-gated command
    /// that the current session cannot execute). Local `/usage` is deliberately
    /// not included here: it reports only this session's local token/context
    /// counters and has no hosted billing semantics.
    ///
...
impl CommandRegistry {
    ...
    pub fn new(builtins: Vec<Arc<dyn SlashCommand>>) -> Self {
        ...
        // Hosted account/product commands stay registered for wire/source
        // compatibility, but are hard-hidden and cannot be dispatched in the
    ...
    /// Compatibility setter for the removed `/share` command. It remains
    /// hard-hidden even if an old caller asks to enable it.
    pub fn set_share_visible(&mut self, visible: bool) {
        let _ = visible;
        self.hidden.insert("share".to_string());
        self.rebuild_triggers();
    }

    /// Show or hide the local `/dashboard` command (feature-flag gating).
    ///
    /// The dashboard is a local session switcher/overview. It is hidden by
    /// default until the local dashboard feature flag is resolved, then
    /// revealed by the agent view. No hosted account or network capability is
    /// implied by this setter.
    pub fn set_dashboard_visible(&mut self, visible: bool) {
        if visible {
            self.hidden.remove("dashboard");
        } else {
            self.hidden.insert("dashboard".to_string());
        }
        self.rebuild_triggers();
    }

    /// Compatibility setter for the removed hosted recap surface.
...
mod tests {
    ...
    fn removed_share_stays_hidden() {
        ...
        // The clean build hard-hides /share from the outset.
        assert!(registry.get("share").is_none());
        assert!(registry.get_for_dispatch("share").is_none());
        assert!(!registry.triggers().iter().any(|t| t.canonical == "share"));

        // A stale upstream enable request cannot resurrect it.
    ...
    fn dashboard_command_hidden_by_default_and_toggleable() {
        ...
        // Enabling the local feature reveals it.
