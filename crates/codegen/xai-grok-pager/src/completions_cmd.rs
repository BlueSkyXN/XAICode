//! `xaicode completions <shell>` — generate shell completion scripts.
//!
//! Used by shell integration and package tooling; must stay side-effect free
...
pub fn run(shell: Shell) {
    // Keep completion output aligned with the default binary name.
    let mut cmd = PagerArgs::command().name("xaicode");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "xaicode", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "xaicode", &mut buf);
    match String::from_utf8(buf) {
        ...
        // unreachable in practice — but package tooling runs this command, so
...
/// `$line[2]` empty, and the dispatch falls through — so `xaicode worktree <TAB>`
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:xaicode-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:xaicode-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("xaicode");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "xaicode", &mut buf);
    ...
    // so `xaicode worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
