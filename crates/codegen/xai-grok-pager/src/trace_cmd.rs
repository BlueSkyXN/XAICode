...
pub async fn run(args: TraceArgs, agent_config: &AgentConfig) -> Result<()> {
    // Trace export is a useful local debugging aid, but uploading a session
    // archive is a hosted-product integration.  Keep the original command
    // and its local archive format while making every invocation fail closed
    // to a local export; stale config/env values cannot re-enable upload.
    if !args.local && !args.json {
        eprintln!("Remote trace upload is disabled in the clean local build; exporting locally.");
    }
    run_export(
        &args.session_id,
...
async fn run_upload(
    ...
    if !cfg!(test) {
        let _ = (session_id, output, json, agent_config);
        anyhow::bail!("remote trace upload is disabled in the clean local build");
    }
    let session_dir = find_session_dir(session_id)?;
    if !json {
        eprintln!("Found session at: {}", session_dir.display());
    ...
    let upload_method = match upload_method {
        ...
        None => {
            ...
            anyhow::bail!(
                "No upload credentials. Remote trace upload is disabled in the clean local build; \
                 local session export is available instead."
            );
        }
    };
