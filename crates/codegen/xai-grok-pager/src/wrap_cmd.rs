//! `xaicode wrap` — run any command in a local PTY that forwards its clipboard.
//!
...
//! run — a single shell-quoted string (`xaicode wrap "mycli ssh host"`) or a shell
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("xaicode wrap: no command given"))?;
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("xaicode wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `xaicode wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`xaicode wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`xaicode wrap "$PROG" ...` with `$PROG` unset)
...
///
/// Unlike the removed hosted transport, `xaicode wrap` does not gate on the terminal brand: the user
