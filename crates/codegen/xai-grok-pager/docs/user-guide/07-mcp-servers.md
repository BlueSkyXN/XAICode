...
MCP (Model Context Protocol) servers extend XAICode with external tool integrations. They let XAICode interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to XAICode over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside XAICode's built-in tools. The model can discover and call these tools during a session.
...
XAICode spawns a local process and communicates over stdin/stdout:
...
>
> - env `GROK_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; XAICode-native
...
xaicode mcp list
xaicode mcp list --json          # Machine-readable output
...
xaicode mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
xaicode mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
xaicode mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
xaicode mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
xaicode mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
xaicode mcp remove github

# Enable or disable a local/TOML (or compat-sourced) server
xaicode mcp enable github
xaicode mcp disable github

# Diagnose a server's configuration and connectivity
xaicode mcp doctor               # Check every configured server
xaicode mcp doctor github        # Check one server
xaicode mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `xaicode mcp add` writes to `~/.grok/config.toml` (`--scope user`). Use `--scope project` to write to `.grok/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `xaicode mcp list` shows servers from both scopes, marking project-scoped ones with `(project)` and disabled ones with `(disabled)`.

`xaicode mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.

`xaicode mcp enable` / `disable` persist the personal on/off state to user `~/.grok/config.toml` (`disabled_mcp_servers`, and `[mcp_servers.<name>].enabled` when that entry exists). Scope:

- **Known names:** user/project XAICode TOML, names already on the disabled list, compat sources (`.mcp.json`, Claude, Cursor), and **plugin** MCP servers (same discovery as doctor/`/mcps`).
...
- **Hosted gateway unavailable:** gateway connector identifiers (`managed_gateway:…`) remain
  wire-compatibility data only and cannot be enabled in XAICode. The command is idempotent;
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. XAICode handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers XAICode's own OAuth client with the provider.

XAICode walks from the current directory up to the git repo root, loading `.grok/config.toml` at each level:
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. XAICode reads most other config sections only from `~/.grok/config.toml`.
...
You can enable or disable MCP servers without restarting XAICode (TUI `/mcps` or CLI — see [CLI Management](#cli-management)).
...
XAICode loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native XAICode config | `~/.grok/config.toml`, `.grok/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.grok/config.toml` or the corresponding environment variable (`GROK_CURSOR_MCPS_ENABLED`, `GROK_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `xaicode inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, XAICode handles the credential flow automatically. When an MCP server requests OAuth credentials, XAICode opens a browser-based authorization flow and stores the resulting tokens for future use.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). XAICode expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
1. The parent session actually connected the server (see Extensions / `xaicode inspect`)
...
For stdio servers, XAICode captures the process's standard error to `~/.grok/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:
...
Use `xaicode inspect` to see all loaded MCP servers and their sources:

```bash
xaicode inspect          # Human-readable
xaicode inspect --json   # Machine-readable
```

### Debug Logging

```bash
RUST_LOG=debug GROK_LOG_FILE=/tmp/grok.log xaicode
tail -f /tmp/grok.log
```
