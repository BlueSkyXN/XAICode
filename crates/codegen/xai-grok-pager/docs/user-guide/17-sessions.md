...
Sessions are identified by a unique session ID (a UUIDv7 by default) and stored
under `~/.grok/sessions/`. Set `GROK_HOME` to override the base directory;
this upstream-compatible path variable is retained for local data migration.
...
```bash
xaicode --resume <session-id-or-title>
```
...
When you launch `xaicode`, the welcome screen lists recent sessions for the current directory. Select one to resume it.
...
- Shell version
- Configured provider mode (API key or model-local credentials)
- Session ID
- Working directory
...
xaicode -p "Continue where we left off" -r <session-id-or-title>

# Continue the most recent session in the current directory
xaicode -p "What were we doing?" -c
```
