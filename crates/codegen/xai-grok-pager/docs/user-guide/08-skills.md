...
Skills are reusable prompt packages that extend XAICode with task-specific instructions. They let you capture a repeatable procedure once, instead of re-explaining it each session.

---

## What Are Skills?

A skill is a directory that contains a `SKILL.md` file. Its markdown body tells XAICode how to handle a specific type of task: step-by-step instructions, conventions, and tool-usage patterns.

Use a skill for a repeatable procedure that's too specific for AGENTS.md but too long to retype. XAICode activates a skill only when it applies to your current task.

---

## Skill Locations

XAICode discovers skills from these directories, in priority order:
...
XAICode deduplicates skills by name -- a higher-priority location overrides a lower one. XAICode also scans `.agents/skills/` (and `commands/`) at each tier (alongside `.grok/`) and walks every directory between your working directory and the repo root.
...
XAICode scans the Claude and Cursor skill directories by default. To stop scanning a vendor, set its `skills` cell to `false` under `[compat.cursor]` or `[compat.claude]` in `~/.grok/config.toml`, or set the `GROK_CURSOR_SKILLS_ENABLED` or `GROK_CLAUDE_SKILLS_ENABLED` environment variable to `false`. See [Configuration](05-configuration.md#harness-compatibility) for details. XAICode always filters out known vendor-shipped default skills (such as Cursor's `shell`, `canvas`, and `statusline`), regardless of these settings.
...
Each entry in `paths` is a `SKILL.md` file or a directory that XAICode walks recursively. `ignore` hides a skill completely; `disabled` keeps it in the list but excludes it from the system prompt and from invocation. `paths` and `ignore` take filesystem paths and support `~` expansion; `disabled` takes skill names.
...
| Field | Description |
|-------|-------------|
| `name` | Skill identifier. Use lowercase letters, digits, and hyphens, up to 64 characters. XAICode normalizes spaces and underscores to hyphens. If you omit `name`, XAICode uses the skill's directory name. |
| `description` | What the skill does and when to use it. XAICode reads this to decide whether to invoke the skill. If you omit it, XAICode uses the first paragraph of the body. |

Write a specific `description`. It determines when XAICode invokes the skill automatically. Name the trigger phrases and use cases.
...
| `metadata` | Arbitrary string key-value pairs. XAICode promotes `metadata.author` and `metadata.short-description` for display. |

---

## Creating Skills with /create-skill

The `/create-skill` command walks you through building a new skill interactively. XAICode asks what you want, drafts the files, and writes them to disk.

### How It Works

When you run `/create-skill`, XAICode:

1. **Gathers requirements.** XAICode asks for the skill name, the scope to save it under, and a description of the workflow you want to capture. Use a name with lowercase letters, digits, and hyphens (2–64 characters, starting and ending with a letter or digit).

2. **Drafts the description.** XAICode writes a `description` that states what the skill does, the phrases that trigger it, and the slash command name. You approve or edit the draft before continuing.

3. **Creates the skill directory.** XAICode creates the `<scope>/.grok/skills/<name>/` directory, plus `scripts/` or `references/` subdirectories when the skill needs them.

4. **Writes SKILL.md.** XAICode writes the frontmatter (`name` and `description`) and a markdown body of instructions, along with any supporting files.

5. **Verifies and confirms.** XAICode reads the file back, confirms it wrote correctly, and tells you how to run the skill.

### Choosing a Scope

XAICode asks where to save the skill:

- **Project** (`<repo_root>/.grok/skills/<name>/`) -- available only in this repository and shareable with teammates through version control. XAICode recommends this scope inside a git repository.
...
The new skill appears in the slash menu within a few seconds, because XAICode reloads skills when files change on disk.
...
To browse your skills, type `/` to open the slash-command menu. XAICode lists every built-in command and skill and filters them as you type. To list skills from the command line instead, run `xaicode inspect` (see [Viewing Skill Details](#viewing-skill-details)).

### Qualified Names

When a skill's name collides with another skill or a built-in command, XAICode keeps **both** invocable. The built-in keeps the bare name (`/login`, `/compact`, …). The skill is advertised under a qualified name prefixed by its scope -- `local:`, `repo:`, `user:`, or the plugin name:
...
`xaicode inspect` tags colliding skills with `[collides with /login → /acme:login]`.

### Automatic Invocation

XAICode can invoke a skill on its own when it recognizes a relevant task. XAICode matches your prompt against the skill's `description` and `when-to-use` fields, so write both to describe the triggering situation.
...
Run `xaicode inspect` to see every skill XAICode discovers, along with the rest of your configuration:

```bash
xaicode inspect          # Human-readable summary
xaicode inspect --json   # Machine-readable report
```

In the human-readable output, the Skills section lists each skill's name and its source -- `project`, `user`, `bundled`, `config` (a `[skills].paths` entry), `server` (skills synced from the skill store in managed workspaces), or `plugin: <name>`. XAICode tags any skill disabled via `[skills].disabled` or from a disabled vendor surface with `[disabled]`.
...
XAICode distributes platform skills separately from your personal skills. Bundled skills are cached under `~/.grok/bundled/skills/`; XAICode never writes them into `~/.grok/skills/`. A same-named local, repo, or user skill overrides the bundled copy. `xaicode inspect` labels each definition by its actual source. (A plugin skill of the same name does not override a native skill; it stays available under its qualified `plugin:name` form.)

Skills can also come from plugins. When you install a plugin that includes skills, they appear alongside your user and project skills. `xaicode inspect` labels each plugin-provided skill with its source as `plugin: <name>`.
...
2. **Include concrete steps.** Skills work best when they give XAICode a clear, ordered procedure to follow.
