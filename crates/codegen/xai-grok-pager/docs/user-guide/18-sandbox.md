...
Under `workspace`, `read-only`, and `strict` (and custom profiles that extend those bases), the XAICode state directory remains writable for session/runtime files, but the kernel **write-denies** the XAICode-owned direct disk paths used as user-global hook sources (they stay readable):
...
If the user and project files define the same custom profile differently, XAICode uses the user profile and shows a startup warning. Run `/doctor` to see both file locations and how to resolve the conflict. Identical definitions do not produce a warning.
...
> `bubblewrap`: if it is missing (or any single deny path can't be bound), XAICode
...
> metacharacter, or one that is malformed, makes XAICode **refuse to start** (fail
...
>   so matching files are denied **even if created after XAICode starts**.
...
>   files, or whose tree is too deep or broad to scan, makes XAICode **refuse to
...
The sandbox is applied to the **entire xaicode process** at startup using kernel primitives -- not per-command wrapping. This means all tool operations are covered:
...
- If a built-in profile fails to apply, XAICode warns and continues without enforcement (see [Platform Support](#platform-support)), but still refuses the leader so tools are not delegated elsewhere
- `xaicode workspace start`, `restart`, and `resume` are unavailable; `pause`, `stop`, and `status` still work
...
for the life of the session**. When you resume it (`xaicode --resume <id>`,
`xaicode --continue`, or `xaicode -r`), XAICode restores that same profile automatically —
...
If the sandbox cannot be applied (e.g., unsupported kernel, missing entitlements), XAICode logs a warning and continues without enforcement. The exception is an explicitly-requested **custom profile**: on **both macOS and Linux**, if it cannot be applied (unknown profile, malformed `sandbox.toml`, or — on Linux — `bubblewrap` unavailable for a non-empty `deny`), XAICode refuses to start rather than run with its denied paths exposed.
