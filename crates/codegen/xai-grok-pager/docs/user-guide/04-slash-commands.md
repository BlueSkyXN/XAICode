...
The hosted feedback endpoint is removed. The command is kept as a hidden
compatibility no-op; report issues through your normal local project workflow.
...
- Bare `/docs` (or `/docs how-to`) opens the local How-to Guides picker.
- `/docs web` is unavailable because the clean build has no hosted docs link.
...
Show locally recorded session token/context usage. Alias: `/cost`. This never
queries account credits, subscriptions, or billing.

```
/usage
```

`/login`, `/logout`, and hosted privacy/account controls are not available in
the local build. Configure a provider key through environment/configuration.
