...
2. **Environment variables** (e.g. `CODING_AGENT_API_KEY`, `GROK_MEMORY`)
...
```toml
[cli]
# Automatic update checks are removed; this table is retained for compatibility.

[models]
default = "local-model"                # model used for new sessions
web_search = "local-model"             # compatibility value; hosted search is off
...
remote_fetch = false                   # hosted model/settings fetch is disabled
...
See [Authentication](02-authentication.md). The clean runtime uses a generic
provider API key from `CODING_AGENT_API_KEY`, `OPENAI_API_KEY`, or a model's
`api_key`/`env_key`. Browser login, OIDC, external account refresh, and cached
account files are disabled.
...
env_key = "MY_CODER_API_KEY"          # env var(s) holding the API key
...
Credential resolution: `api_key` > `env_key` > `CODING_AGENT_API_KEY` >
`OPENAI_API_KEY`. See [Custom Models](11-custom-models.md) for query params
and extra headers, and [Sandbox Mode](18-sandbox.md#shell-environment-policy)
for environment inheritance controls.
...
[subagents.models]
explore = "local-model"              # route to different models
```
...
Product analytics, Sentry/Mixpanel, OTLP forwarding, trace upload, feedback
submission, and remote unified-log forwarding are disabled in this build.
Legacy `[telemetry]` keys are accepted only for configuration compatibility;
they cannot enable network telemetry or trace upload.

### Version pinning

Automatic update checks and installer downloads are removed. Pin or distribute
the binary through your normal package/release workflow instead.

### Enterprise deployment

Use your organization's normal package manager and provider gateway. The clean
runtime does not sync managed settings, model catalogs, telemetry policy, or
remote credentials; local `config.toml` is the source of truth.
...
| Variable | Description |
|----------|-------------|
| `CODING_AGENT_API_BASE_URL` | Override the local/provider inference endpoint |
| `OPENAI_BASE_URL` | Compatibility endpoint fallback |
...
| `~/.grok/auth.json` | Legacy account file; not read by the clean runtime |
