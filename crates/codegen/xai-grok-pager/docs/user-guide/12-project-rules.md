...
Project rules let you configure XAICode per project or directory. By placing an AGENTS.md file in your repository, you can set coding conventions, build instructions, style guides, and any other instructions that XAICode should follow when working in that codebase.

---

## What Are Project Rules?

Project rules are Markdown files that XAICode reads and adds to its context. XAICode follows their content for every interaction in that tree.

This is the primary mechanism for teaching XAICode about your project's conventions, so you need not restate them each session.

---

## Supported File Names

XAICode checks for these filenames (in this order) within each directory:
...
XAICode loads every matching file in a directory, so a folder that contains both `AGENTS.md` and `CLAUDE.md` contributes both. On case-insensitive filesystems, names that resolve to the same file (such as `Agents.md` and `AGENTS.md`) are deduplicated and counted once. `Claude.md`, `CLAUDE.md`, and `CLAUDE.local.md` are supported for compatibility with Claude Code workflows. When Claude compatibility is enabled (the default), XAICode also scans your home-level `~/.claude/` directory for these filenames and, at each directory level, checks `.claude/CLAUDE.md` and `.claude/CLAUDE.local.md` -- the locations Claude Code uses for project memory. With Cursor compatibility enabled, the home-level `~/.cursor/` directory is scanned the same way.

### Rules Directories

In addition to AGENTS.md files, XAICode scans for `*.md` files in rules directories at each level (`<dir>`) from the repo root to the current working directory:
...
XAICode also scans home-level rules, regardless of where it starts. These roots are already vendor-specific, so rules live directly under `rules/`:
...
XAICode scans for project rules in this order:
...
When XAICode runs in `~/projects/my-app/src/components/`, it loads all three files. The instructions accumulate, so XAICode sees all of them.

### Deeper Files Take Precedence

XAICode orders the files from the repo root to the current working directory, so files in deeper directories appear later in its context and take precedence when instructions conflict. In the example above, if the root says "Use styled-components" but `components/AGENTS.md` says "Use CSS modules", the CSS modules instruction wins because it appears later.

### Auto-Loading Behavior

- XAICode loads the files from the repo root to the current working directory automatically at session start.
- When XAICode reads, lists, or edits files in directories outside that initial set, it detects any project instruction files there, notes their paths, and reads them when they apply to the task.
...
```bash
xaicode --rules "Always use TypeScript. Prefer functional components."
```

XAICode appends this text to the session's system prompt. Use it for session-specific customization.

To replace the system prompt entirely, pass `--system-prompt-override` (alias `--system-prompt`). XAICode uses the text verbatim and skips both the default system prompt and `--rules`. (Text passed with `--rules`, by contrast, is wrapped in a `<human_rules>` block and appended to the default prompt.)

---

## File Size

XAICode loads each project instruction file in full; there is no character cap and no truncation. Even so, keep instructions concise and focused. Shorter, specific rules are easier for XAICode to follow than long ones, and every file you load consumes context.
...
As top-level instruction files, XAICode discovers only the recognized filenames listed under [Supported File Names](#supported-file-names) — not custom names such as `AGENTS.local.md` or `notes.md`. (Inside a rules directory such as `.grok/rules/`, every `*.md` file is loaded regardless of name.)
...
Use `xaicode inspect` to see all loaded project instructions:

```bash
xaicode inspect
```

This shows each project instruction file it finds, with its path and approximate token count. Use it to confirm XAICode picks up your rules.
