...
XAICode runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in XAICode to check the current session and see available fixes. If
XAICode cannot start, run `xaicode doctor` in your shell. Use `xaicode doctor --json`
...
`xaicode doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `xaicode doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. XAICode preserves the file's
...
XAICode deliberately does **not** run `tmux source-file` or change the live tmux
...
XAICode detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach XAICode.
...
Inside tmux there are two separate questions: what color XAICode emits, and what
...
XAICode writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when XAICode runs inside tmux.
...
terminal focused. Older compositors may require XAICode to remain focused until
the copy message appears. XAICode shows a startup warning when this applies; run
...
XAICode emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `GROK_CLIPBOARD_NO_OSC52=1` before starting XAICode to
...
A remote XAICode process normally cannot read the local terminal's selection. Use
...
When XAICode cannot identify the outer terminal over SSH, it predicts that OSC 52
...
through `xaicode wrap`, for example `xaicode wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `xaicode wrap`, XAICode shows the one-time tip
...
For repeated SSH use, Doctor offers `xaicode doctor fix ssh-wrap`. It also shows
...
> **Warning**: `xaicode wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. XAICode normally uses
...
`xaicode --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with XAICode

Zellij can intercept Ctrl/Alt keys before they reach XAICode. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach XAICode, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in XAICode.
...
keys. XAICode therefore does not negotiate the protocol there, and Shift+Enter can
...
If XAICode stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, XAICode stops capture and shows
...
to the terminal hosting XAICode. Open **System Settings → Privacy & Security →
...
Run `xaicode doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone XAICode would use. If no input device is available, Doctor
