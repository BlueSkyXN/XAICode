...
Memory lets XAICode recall facts, decisions, and patterns from earlier sessions. XAICode indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each XAICode session starts fresh: the model knows nothing about previous sessions. When you enable memory, XAICode can:
...
XAICode suffixes each workspace directory with a short hash of the repository's identity. The identity is the `origin` remote in `org/repo` form when the directory is a Git repository with an `origin` remote, or the directory path otherwise. Because clones and worktrees of the same repository share an `origin` remote, they also share one memory directory.
...
When a session ends, XAICode saves a structured metadata summary to that session's daily log. The summary contains:
...
XAICode builds the summary from conversation metadata without an LLM call, without added latency. XAICode skips the save for trivial sessions -- those with fewer than three substantive prompts, or fewer than 50 bytes of user text.
...
Ask XAICode to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.grok/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

XAICode records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, XAICode opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, XAICode shows `Memory saved to ~/.grok/memory/MEMORY.md`.

### Forget

Ask XAICode to forget something, and it finds and removes the matching entry:
...
Ask what XAICode remembers:

```
> what do you remember?
```

XAICode searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.
...
When you save a note with `/remember`, XAICode confirms in the scrollback:

```
Memory saved to ~/.grok/memory/MEMORY.md
```

Background saves — flush, dream, and session-end — run silently and do not post a scrollback message. Use `/memory` at any time to browse what XAICode has stored.
...
Dream also runs automatically. By default, XAICode checks the consolidation gates when a session ends and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, XAICode automatically searches memory for content relevant to the current project and injects it as context. This means XAICode starts with knowledge from previous sessions without a reminder.
...
XAICode searches memory automatically, but you can also trigger searches manually in the chat:
...
| `min_score` | unset | Score threshold for first-turn results. When unset, XAICode applies no threshold, which is equivalent to `0.0`. |
...
| `flush_model` | unset | Model for the flush turn. When unset, XAICode uses the session's primary model. |
...
When a session memory is old, XAICode attaches a staleness note to it in search results. Older results get a stronger reminder to verify the current state before you rely on them. These notes help you spot stored facts that might no longer be accurate. Global and workspace memories never receive staleness notes, because they hold curated long-term knowledge.

---

## File Watcher

By default, XAICode watches `~/.grok/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
1. Verify memory is enabled: check `xaicode inspect` output.
2. Check the flag: `xaicode --experimental-memory` or `GROK_MEMORY=1`.
