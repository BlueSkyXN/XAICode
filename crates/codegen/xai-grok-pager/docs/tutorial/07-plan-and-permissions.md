...
XAICode asks before doing anything risky — and can plan before it codes.

## Permissions

When XAICode wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: XAICode explores the
...
`s` requests changes — XAICode iterates until you're happy, then implements.
...
background — XAICode keeps working and you're notified when it finishes
