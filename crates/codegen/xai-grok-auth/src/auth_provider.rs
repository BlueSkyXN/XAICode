...
pub trait AuthCredentialProvider: HttpAuth + Send + Sync + 'static {
    ...
    /// Whether the legacy vendor marker header should be sent with the bearer
    /// token.  The clean production build leaves this off; the test-only
    /// compatibility path can still exercise the upstream wire contract.
