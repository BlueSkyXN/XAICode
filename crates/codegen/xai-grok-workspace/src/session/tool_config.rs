...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    headers.insert("user-agent".to_string(), format!("xaicode/{version}"));
    if cfg!(test) {
        headers.insert("x-grok-client-version".to_string(), version.to_string());
        headers.insert(
            "x-grok-client-identifier".to_string(),
            std::env::var("GROK_CLIENT_NAME").unwrap_or_else(|_| "xaicode".to_string()),
        );
    }
    if cfg!(test) && (base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy")) {
        // Keep the upstream proxy fixture available to compatibility tests;
        // production builds never emit vendor-only authentication headers.
...
/// Build generic web-fetch config. Enabled with default params unless
/// `CODING_AGENT_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> xai_grok_tools::implementations::grok_build::web_fetch::WebFetchConfig
{
    ...
    let disabled = std::env::var("CODING_AGENT_DISABLE_WEB_FETCH")
        .or_else(|_| std::env::var("GROK_DISABLE_WEB_FETCH"))
        .is_ok_and(|v| v == "1" || v == "true");
    if disabled {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("CODING_AGENT_WEB_FETCH_PROXY")
        .or_else(|_| std::env::var("GROK_WEB_FETCH_PROXY"))
    {
        params.proxy_endpoint = Some(proxy);
    }
    let allow_local = xai_grok_config::env_bool("CODING_AGENT_WEB_FETCH_ALLOW_LOCAL")
        .or_else(|| xai_grok_config::env_bool("GROK_WEB_FETCH_ALLOW_LOCAL"));
    if allow_local == Some(true) {
        params.allow_local = Some(true);
    }
    WebFetchConfig::Enabled { params }
}
fn default_web_search_model() -> String {
    std::env::var("CODING_AGENT_WEB_SEARCH_MODEL")
