//! Compatibility entry point for the upstream hosted workspace ToolServer.
//!
//! The clean local build keeps this binary so the original workspace crate
//! and launcher surface remain source-compatible, but it does not start a
//! hosted sandbox, read account credentials, export telemetry, or connect to
//! a remote hub.  Local coding-agent sessions use the normal process/tools
//! path instead.
use clap::Parser;
...
const SERVICE_NAME: &str = "coding-agent-workspace";
...
#[derive(Parser)]
#[command(name = "coding-agent-workspace-server")]
#[command(about = "Compatibility workspace server (disabled in the clean local build)")]
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "ws://127.0.0.1:0/disabled-workspace-hub")]
    hub_url: String,
    #[arg(long)]
...
const CAPABILITIES: Capabilities = Capabilities { diag: false };
fn main() -> anyhow::Result<()> {
    ...
    // Keep the original CLI shape for downstream launchers, but never let
    // this compatibility binary perform the hosted workspace side effects
    // that made the upstream build vendor-specific.
    return Err(anyhow::anyhow!(
        "the hosted workspace server is disabled in the clean local build"
    ));
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    // The public helper is retained for source compatibility with embedders;
    // it is fail-closed as well as the binary's `main` path.
    let _ = (&args, &cwd);
    return Err(anyhow::anyhow!(
        "the hosted workspace server is disabled in the clean local build"
    ));
...
mod tests {
    ...
    fn capabilities_manifest_shape() {
        ...
        assert_eq!(value, serde_json::json!({"diag": false}));
    }
    #[test]
