...
pub async fn supervise_preview(cfg: PreviewArgs, shutdown: watch::Receiver<bool>) {
    if !cfg!(test) {
        // The preview proxy is a hosted workspace companion. Keep the
        // supervisor/test seam for upstream compatibility, but do not spawn
        // its child process in the clean production build.
        let _ = (cfg, shutdown);
        return;
    }
    tracing::info!(
...
pub async fn supervise_preview_activity(
    ...
    if !cfg!(test) {
        let _ = (control_port, tracker, scrape_interval, shutdown);
        return;
    }
