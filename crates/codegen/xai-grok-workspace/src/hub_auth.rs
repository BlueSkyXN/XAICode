...
pub fn provider(
    ...
    // The standalone workspace server is a hosted/account feature. Keep the
    // original auth-provider implementation for tests and downstream source
    // compatibility, but make production unable to read or rewrite the
    // legacy auth.json/OIDC store.
    if !cfg!(test) {
        let _ = (hub_url, auth_config);
        anyhow::bail!("hosted workspace authentication is disabled in the clean local build");
    }
