    ...
    raw_sources
        ...
        .filter_map(|raw| {
            let kind = if let Some(git_url) = raw.git {
                if !cfg!(test) && crate::is_blocked_vendor_source_url(&git_url) {
                    tracing::debug!(url = %git_url, "blocked vendor marketplace source ignored");
                    return None;
                }
                SourceKind::Git {
                    url: git_url,
    ...
    for (name, config) in marketplaces {
        ...
        let kind = match entry.source {
            SettingsSource::Git { url } => {
                if !cfg!(test) && crate::is_blocked_vendor_source_url(&url) {
                    tracing::debug!(url = %url, "blocked vendor marketplace source ignored");
                    continue;
                }
