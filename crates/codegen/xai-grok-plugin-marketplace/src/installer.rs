...
pub fn install_from_marketplace(
    ...
    if !cfg!(test) && crate::is_blocked_vendor_source_url(&provenance.source_url_or_path) {
        return Err(InstallError::InstallFailed {
            detail: "vendor marketplace sources are disabled in the clean local build".into(),
        });
    }
